...
> **版本** v1.2 · **状态** 已同步实现（P7b 契约同步 · 以代码为准）· **日期** 2026-09-16 · **作者/产出** task-decomposer
...
> 基础层/数据层接口权威：`config.md` v1.1 · `metrics.md` v1.1 · `stock_api.md` v1.0（`cache.md` 与本模块**无接口**）
...
1. **推送主循环 `push_loop`**：每 L1 tick 一次「清扫僵尸组 → 取活跃码集 → 分片轮转刷新 → 快照广播」；**tick 预算基准 = 本轮开始时刻**（修 R8 的静默滑动），耗时/滑动/degraded 入 metrics。
2. **分片轮转刷新 `_refresh_pool(codes, now=None, fields=None, tick=None, deadline=None)`（R-6 / AR-1 / INV-1b）**：按**订阅字段并集**刷新（`_subscribed_fields`/`_active_targets`/`_resolve_refresh_fields`）；条件 `C1: _fetches_per_code(fields) × N_active ≤ coverage` 成立 ⇒ 整池一周期（**lag=0**）；否则每 tick 只刷 `|slice| ≤ coverage_codes`（盘中 quote-only **23** / 2 域 **11** / 3 域 **7**），其余码从**终点缓存**读取（`stock_api.cached_batch`），并记录 `stream_refresh_lag_ticks = ceil(n/|slice|)`。
...
```
允许 import：
  标准库：json / logging / queue / socket / threading / time / uuid /
          concurrent.futures.ThreadPoolExecutor / http.server / urllib.parse
  包内  ：config / metrics / stock_api（handle_cls_basic_infos, handle_cls_fundflow, handle_cls_timeline,
...
|---------------|-------------------|
| `handle_cls_basic_infos / handle_cls_fundflow / handle_cls_timeline(codes, deadline=None, dropped=0) -> dict` | `_FIELD_HANDLERS = {'quote': handle_cls_basic_infos, 'fundflow': handle_cls_fundflow, 'timeline': handle_cls_timeline}`（**不变**）；调用 `handler(list(sl), deadline=deadline)`（★ 贯通 tick 预算；**不传 `dropped`** —— 2000 码内部已分块，截断只属 HTTP 层） |
...
| `BATCH_MAX_WORKERS = 8`（公开名） | `coverage` 计算（§3.4） |
...
```yaml
openapi: 3.0.3
info: {title: china-finance-rss stream port (8054), version: '1.3'}
paths:
    ...
    post:
      ...
      responses:
        '201': {description: 建组成功, content: {application/json: {example: {sid: ab12cd34ef56, codes: [sh600519], fields: [quote],
                 refresh_capacity_codes: 23, refresh_lag_ticks: 9, capacity_warning: '...'}}}}
...
# ── 刷新 / 广播 / 帧 ────────────────────────────────────────────
def tick_interval() -> int                                        # _trading_tiers()['L1']（不变）
def _fetches_per_code(fields=None) -> int                         # ★ Σ _FIELD_FETCH_CALLS
...
def _broadcast(snapshot) -> None                                  # 签名不变；先过滤 closed
def _tick_sleep_seconds(t0, tick, now) -> float                   # ★ 整 tick 网格
def _push_once(t0=None) -> float                                  # ★ 单轮（可测），返回本轮 sleep
def push_loop() -> None                                           # 异常退避 1/2/4…封顶 8 tick
...
| `now` | `float \| None` | `None` | epoch 秒（注入点）；`None` → `time()`。**同时转发给 `cached_batch(..., now=now)`**（测试可驱动 C2 真过期路径） |
| `fields` | `Iterable[str] \| None` | `None` | 本 tick 要刷新的字段集；`None` → `_subscribed_fields() or list(_FIELD_HANDLERS)`（`_resolve_refresh_fields`） |
| `tick` | `int \| None` | `None` | 调度器一次算好传回（`_push_once` 传 `tick_interval()`），避免 tier 翻转时 C1/C2 门限与派发节拍不一致 |
...
if tick is None: tick = tick_interval()      # 盘中 8 / 非盘中 120
...
for field in fields:                                           # fresh（≤ coverage 取数次数）
    ...
    if time.time() >= deadline:                               # ★ 超预算 ⇒ 跳过剩余字段并标记
        ...
        break
    fetched = handler(list(sl), deadline=deadline)            # ★ 贯通 deadline；不传 dropped
...
- **`_capacity_meta(codes, fields)`（v1.2）**：`refresh_capacity(tick_interval(), fields)` → `refresh_capacity_codes`；`n > cap` 时追加 `refresh_lag_ticks = ceil(n/cap)` 与 `capacity_warning`（键为**增量**，既有 `sid`/`codes`/`fields` 契约不变）。
...
SubscriptionGroup:                    # _groups: dict[sid -> SubscriptionGroup]
  ...
  last_push_ts: <float>               # 最近一次成功广播时刻（僵尸组清扫依据）
  created_ts: <float>
...
_PER_FETCH_EST: 2.2                    # ★ v1.2 重标定：单次 worker-call 的串行等效秒（BUG-P6C-06）
_FIELD_FETCH_CALLS: {quote: 1, fundflow: 1, timeline: 1}   # ★ 每码每域的上游 REST 调用数（单一权威）
...
BATCH_MAX_WORKERS: 8                   # ← stock_api 公开名（import，不重定义）
_STREAM_REFRESH_KEY: 'stream_refresh'  # ← 轮转游标键（stock_api._prefetch_cursor 内）
coverage        = max(1, int(0.8 × tick × BATCH_MAX_WORKERS / 2.2))     # 取数次数/tick
fetches_per_code = Σ _FIELD_FETCH_CALLS[f] for f in fields
coverage_codes  = max(1, coverage // fetches_per_code)                  # 每 tick 可"整码全刷"的码数
# 派生（运行时实值，★ 与 tick 无关的 coverage）：
#   任意 tick → coverage = int(0.8*8*8/2.2) = 23 取数次数/tick
#   coverage_codes（盘中同值；仅随 fields 变）：
#     1 域（quote-only）→ 23 // 1 = 23     2 域 → 23 // 2 = 11     3 域 → 23 // 3 = 7
#   C1: fetches_per_code × n <= coverage  ⇔  n <= coverage_codes（1/2/3 域 = 23/11/7）
```

> **与 SAD 的口径差**：SAD §2.1 INV-1b 的 `coverage≈170 ⇔ coverage_codes≈56` 是**旧标定**（`_PER_FETCH_EST=0.3`、quote 记 2 次调用）。v1.2 实测重标定为 `coverage=23`、`coverage_codes` 随订阅字段为 **23/11/7**（登记 §10#15）。切片上限**随 `tick_interval()` 与订阅字段动态计算**，不写死 56。
...
| **BR-STR-9** | 入队回调顺序固定：**先 `put_nowait` 成功、再 `_frame_acquire`**；`queue.Full` 且二次入队仍失败 ⇒ **不 acquire**（未入队不得计费） | 计费自洽 |
...
| 编号 | 规则 | 依据 |
|------|------|------|
| **BR-STR-16** | `coverage` 单位 = **取数次数/周期**（不是码数）：`coverage = max(1, int(0.8 × tick × BATCH_MAX_WORKERS / _PER_FETCH_EST))`（`_PER_FETCH_EST=2.2` ⇒ **恒 23**）；`coverage_codes = max(1, coverage // _fetches_per_code(fields))` ⇒ **1/2/3 域 = 23/11/7**。`_FIELD_FETCH_CALLS` 是该成本模型的**单一权威**（`refresh_capacity`/`_fetches_per_code` 共用） | SAD §2.1 INV-1b（P1-N1 修；BUG-P6C-06 重标定） |
...
| **BR-STR-27** | **整 tick 网格滑移**（`_tick_sleep_seconds(t0, tick, now)`）：`k = floor((now − t0)/tick) + 1`（≥1），`delay = max(t0 + k×tick − now, 0.0)`。**基准仍 = 本轮开始时刻**（消除静默滑动），且 sleep 恒到 `now` 之后的下一个网格点 ⇒ **间隔永短于一整 tick**（消除"超预算后 2s 连发重复帧 + 前 12–17s 停顿"的双峰）。**旧 `_TICK_MIN_SLEEP_FRACTION=0.25` 已删除**；`delay > 0` 才 `sleep` | SAD §2.6（修 R8）+ BUG-P6C-06 |
| **BR-STR-28** | `_sweep_idle_groups` 每 60s 触发一次（`now - _last_group_sweep >= 60`）；单轮逻辑抽为 **`_push_once(t0=None) -> delay`**（含 `if codes:` 守卫与空池 lag=0 分支，可被测试直驱而不进无界循环）；`push_loop` 内 `try/except Exception` 兜底：连续失败 `consecutive += 1`，`spans = min(2^(consecutive-1), _PUSH_ERROR_BACKOFF_CAP_TICKS=8)`，`delay = max(spans×tick − elapsed, _tick_sleep_seconds(...))` ⇒ **退避 1/2/4/…封顶 8 tick**，绝不退化成 ~1s 自旋 | 现状保留 + P2 退避 |
...
def _refresh_pool(codes, now=None, fields=None, tick=None, deadline=None):
    ...
    for field in fields:                                           # fresh（≤ coverage 取数次数）
        ...
        if time.time() >= deadline:                                # ★ 超预算 ⇒ 跳过剩余字段
            ...
            break
        fetched = handler(list(sl), deadline=deadline)             # BR-STR-24：不传 dropped
        if not fetched:
            continue
        for code, data in fetched.items():
            if not isinstance(code, str) or code.startswith('_'):
...
def _broadcast(snapshot):
    """BR-STR-2/9/10/11/13：锁外帧构建 + distinct 计费 + 确定性丢弃。"""
    now = time.time()
    with _groups_lock:
...
def push_loop():
    """BR-STR-25..28：tick 网格 + 异常指数退避（1/2/4…封顶 8 tick）。"""
    consecutive = 0
    while True:
        t0 = time.time()
        tick = tick_interval()
...
| **STREAM-T12** C1 整池刷新 | `tick=8`、**quote-only**、活跃码 23（`1×23 ≤ coverage=23`）⇒ `_prefetch_slice` **未被调用**、`cached_batch` **未被调用**、`stream_refresh_lag_ticks == 0`；handler 收到全部 23 码 | **A3 / E5** |
| **STREAM-T13** C2 分片轮转 | `tick=8`、3 域、活跃码 200（`3×200=600 > 23`）⇒ handler 只收到 `≤coverage_codes==7` 码（一次 tick）；`cached_batch` 被调 `len(fields)==3` 次（每字段）且收到其余码；`stream_refresh_lag_ticks == ceil(200/7) == 29` | **AR-1 / A3 / E5** |
| **STREAM-T13b** 字段并集刷新（BUG-P6C-06） | 仅 quote 组 ⇒ `_refresh_pool` 只调 `_FIELD_HANDLERS['quote']` **1 次**、不调 fundflow/timeline；`coverage_codes == 23`（非 7） | AC-E5（成本模型） |
| **STREAM-T14** 游标推进 | 连续 `ceil(n/coverage_codes)` tick（同一 200 码集、3 域 ⇒ 29）⇒ 各切片**互不重叠**且合并 == 全集；再一 tick 回到第 1 片（round-robin）；`_prefetch_cursor['stream_refresh']` 按 `coverage_codes`（=7）递增取模 | AR-1 |
...
| **STREAM-T42** 容量元数据（v1.2） | 23 码 quote-only ⇒ 201 含 `refresh_capacity_codes==23`、无 `refresh_lag_ticks`；200 码 3 域 ⇒ 含 `refresh_lag_ticks` 与 `capacity_warning` | **A7 / P1-4** |
...
| 15 | `coverage` 与 tick / 订阅字段的关系 | SAD §2.1 INV-1b 只给盘中口径（170 / 56） | ★ v1.2：`_PER_FETCH_EST=2.2` ⇒ `coverage = int(0.8×tick×8/2.2)` **与 tick 无关（恒 23）**；`coverage_codes` 由**订阅字段**决定（1/2/3 域 → 23/11/7）。旧"非盘中 2560/853"口径作废 | SAD 的 170/56 属旧标定（`_PER_FETCH_EST=0.3` + quote 记 2 次）；**登记以防评审按 56 或 853 硬断言**（BUG-P6C-06） |
...
| 21 | **容量模型重标定**（v1.2） | SAD §2.1 INV-1b：`coverage≈170`、`coverage_codes≈56`（`_PER_FETCH_EST=0.3`、quote 记 2 次调用） | `_PER_FETCH_EST=2.2`、`_FIELD_FETCH_CALLS['quote']=1` ⇒ `coverage=23`、`coverage_codes`=1/2/3 域 → **23/11/7** | 实测冷路径 ≈2.2s/worker-call（20 码×3 域冷 = 10.5s @8 worker）；旧标定下 51 取数 tick 实测 ≈14s > 6.4s 预算（BUG-P6C-06）。**登记以防评审按 56 硬断言** |
...
- [x] 测试要点 **43 条**映射 PRD AC（含 AR-7 陷阱点、预算驱逐、竞态、lag、`MAX_GROUPS` 400；v1.1 新增 T33/T34；**v1.2 新增 T13b/T35–T43**）
- [x] AC 追溯：A1/A2/A3/A7/A10/E5/E7/S2/S7/S9/S10/S11 主承载
- [x] 与 `stock_api` 跨模块契约逐项对齐（`_PROGRESS.md` §A）：handler 组装体（+`deadline`）/ `cached_batch(domain, rest, now)` / `BATCH_MAX_WORKERS` / 轮转原语 / `_` 前缀
- [x] 偏差 **28 项**全部登记（**不改 SAD / 不改 PRD / 不改 config.md / 不改 stock_api.md**）
