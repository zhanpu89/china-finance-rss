...
| **MKT-T20** 终端缓存发布 `cache_entries{margin}`（★ v1.9 / CR-02 · ★ v1.10 / CR-07） | ① **写入**：`metrics.reset()` 后成功取一次 ⇒ `snapshot()['cache_entries']['margin'] == 1`；② **命中**：同 `market` 再调 ⇒ **再次发布**恰 1 次——★ **v1.10（CR-07）**：用 **surrogate spy 计数**（包装 `metrics.set_gauge` 收集发布次数，写入后 `publishes.clear()` **隔离命中路径**，命中后断言 `cache_entries{margin}` **恰好 1 次**）——**裸值 `== 1` 无法检出命中路径漏发，改 spy 计数使其可证伪**（删命中路径 `set_gauge` 即红）；③ **LRU 淘汰**：取满 `cache_max` 后再写第 `cache_max+1` 项 ⇒ `cache_entries{margin} == cache_max` **且** == `len(_margin_cache)`；④ **锁纪律**：两处 `set_gauge` 均在**释放 `_margin_cache_lock` 之后**（复用 `_lock_held_during` 一类断言，或断言发布值 == 真实长度）。落地 = `tests/test_data_layer.py::test_terminal_cache_publishes_cache_entries_gauge` | BR-MKT-17 / AC-S10 / AC-E6 |
...
- [x] **v1.10（CR-07 · MKT-T20 命中断言可证伪化 · 只改文档）**：`test_terminal_cache_publishes_cache_entries_gauge` 的**命中分支断言不可证伪**（删命中路径 `set_gauge` 仍绿——写路径已令 `cache_entries{margin}==1`，裸值无法区分命中的发布）⇒ 测试改 **surrogate spy 计数**：包装 `metrics.set_gauge` 收集发布次数、写入后 `publishes.clear()` **隔离命中路径**、命中后断言 `cache_entries{margin}` **恰好 1 次**（删 `set_gauge` 即红）；§8 **MKT-T20 ② 同步 spy 表述**；**BR-MKT-17 / 对外契约零变更**（锁外发布纪律未改）。**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`_PROGRESS.md` 同步（B-6 闭环）**
...
| v1.10 | 2026-09-21 | **CR-07 · MKT-T20 命中断言可证伪化（只改文档）**：`test_terminal_cache_publishes_cache_entries_gauge` 命中分支改 **surrogate spy 计数**（包装 `metrics.set_gauge` 收集 `cache_entries{margin}` 发布，写入后 `publishes.clear()` 隔离命中路径、命中后断言恰 **1 次**；**删命中路径 `set_gauge` 即红**——裸值 `==1` 无法检出命中路径漏发）；§8 **MKT-T20 ② 同步 spy 表述**（"命中后仍 == 1" → "命中后再次发布恰 1 次"）。**BR-MKT-17 / 对外契约 / BR / 测试编号零变更**；**测试已修改并全绿（526）**；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`_PROGRESS.md` 同步（B-6 闭环）** |
