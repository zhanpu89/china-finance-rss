...
> **版本** v1.2 · **状态** 已契约同步（P7b：以 `china_finance_rss/stock_api.py` 实现为准回写）· **日期** 2026-09-16 · **作者/产出** task-decomposer
...
2. **期限贯通（R13 / ADR-009）**：全部取数器签名统一带 `deadline`，从 handler → `_handle_cached_batch` → `_process_chunk` → `_run_batch` → `_fetch_one` → fetcher → `urlopen/evaluate_fetch` **一路传递**，消除"`TypeError` 回退 ⇒ 期限静默丢弃"。
...
10. **detail 派生行业名缓存（P7b / BUG-P6C-01）**：`_basic_sector_cache` + `_basic_sector_get/put`——`fetch_cls_basic_info` 阶段 2 的行业名二次 REST 调用被 7 天 `sector` 策略缓存吸收，稳态 quote 成本回到 1 调用/码。
...
允许 import：
  ...
  包内  ：config（cache_policy, canonical_code, REQUEST_TIMEOUT, MAX_DEDUP_CODES, _MAX_BATCH_SIZE,
                  *_URL/_HEADERS, _F10_EXPECTED_KEYS, stock_nav_page_names, cdp_engine 槽）
...
> **命名与签名总约定**：码级取数器一律 `fetch_*(code, deadline=None, ttl=None)`；`ttl=None` ⇒ fetcher 内部回退 `cache_policy(domain)['ttl']`（保证直接调用亦同源）。非码取数器不在此列。

### 2.1 上游取数器全清单（R13 期限贯通）

| # | 签名 | 域 | 上游路径 | `deadline` 用法 | `ttl` 用法 |
|---|------|----|---------|----------------|-----------|
| 1 | `fetch_cls_fundflow(stock_code, deadline=None, ttl=None) -> Any \| None` | `fundflow` | REST `_FUNDFLOW_BASE_URL` → CDP `evaluate_fetch` 回退 | 入口判定 + `_evaluate_fetch_any(deadline=)` | `fetch_json(ttl=ttl)` |
| 2 | `fetch_cls_timeline(stock_code, deadline=None, ttl=None) -> Any \| None` | `timeline` | REST `_TIMELINE_BASE_URL` → CDP 回退 | 同上 | 同上 |
| 3 | `fetch_cls_f10(stock_code, deadline=None, ttl=None) -> dict \| None` | `f10` | **CDP 导航**（`_navigate_f10` + `cdp_engine.page_data`） | 导航/求值预算 `d = deadline or time()+_CDP_CALL_TIMEOUT` | 参数保留、**不使用**（无 URL 缓存） |
| 4 | `fetch_cls_basic_info(stock_code, deadline=None, ttl=None) -> dict \| None` | `quote` | REST `_BASIC_INFO_BASE_URL`（阶段 1）+ `_STOCK_DETAIL_BASE_URL`（阶段 2 行业） | 入口判定 + 两阶段各自的 `_fetch_rest_json(deadline=)` | 两阶段 `fetch_json(ttl=ttl)` |
| 5 | `fetch_cls_announcement(stock_code, deadline=None, ttl=None) -> Any \| None` | `announcement` | REST sign URL → CDP 回退 | 同 #1 | `fetch_json(ttl=ttl)` |
| 6 | `fetch_cls_stock_detail(stock_code, deadline=None, ttl=None) -> Any \| None` | `quote` | REST `_STOCK_DETAIL_BASE_URL` | 入口判定 + `_fetch_rest_json(deadline=)` | `fetch_json(ttl=ttl)` |

**prefetch 专用取数器**（CDP 浏览器上下文 anti-ban 优先，REST 腿**经 `fetch_json`**，P7b）：

| # | 签名 | 域 | 说明 |
|---|------|----|------|
| 7 | `_fundflow_direct_fetch(stock_code, deadline=None) -> Any \| None` | `fundflow` | CDP `evaluate_fetch` 优先 → **`_fetch_rest_json`（即 `fetch_json`）回退**；失败 `raise FetchError`；`upstream_fetch_total` **单次逻辑取数只计 1 次** |
| 8 | `_timeline_direct_fetch(stock_code, deadline=None) -> Any \| None` | `timeline` | 同 #7 |
| 9 | `_announcement_direct_fetch(stock_code, deadline=None) -> Any \| None` | `announcement` | 同 #7 |
...
```python
def _call_fetcher(fetcher, code, deadline, ttl) -> Any
def _fetch_one(fetcher, code, deadline=None, ttl=None) -> tuple[Any | None, str | None]:
```
...
**`_call_fetcher` 的降级规则（P7b / P2-④，取代 v1.1 的"显式回退 + log.debug"）**：按 4 档关键字形式依次尝试 `{deadline, ttl}` → `{deadline}` → `{ttl}` → `{}`，**仅在"调用帧" TypeError**（参数绑定失败，即签名不匹配的测试替身）时才降级到下一档；若 TypeError 的 traceback 有 `tb_next`（说明异常抛自 fetcher **函数体内**）则**原样上抛**——在体内 TypeError 上重试会**把取数执行两次**。每档降级都保留 callable 能接受的 `deadline`/`ttl`，**预算永不被静默丢弃**。四档全被拒 ⇒ `raise TypeError(f'{fetcher!r} does not accept (code, deadline, ttl)')`。
...
```python
def handle_cls_fundflow(codes, deadline=None, dropped=0) -> dict
def handle_cls_timeline(codes, deadline=None, dropped=0) -> dict
def handle_cls_f10(codes, deadline=None, dropped=0) -> dict
def handle_cls_basic_infos(codes, deadline=None, dropped=0) -> dict
...
- `domain` ∈ `{quote, fundflow, timeline, f10, announcement}`；未知 domain → `{}` + `log.warning`（不抛）。
...
| 名称 | 变更 |
|------|------|
| `handle_cls_stock(stock_code) -> Any \| None` | **保留**（`server.py:55` import）；改为 `fetch_cls_stock_detail` 的**薄封装**（吞 `FetchError` 返回 `None`，`handle_*` 约定） |
...
| 5 个终点缓存容器名与 5 个 pool 名 | **保留**；容器类型 `dict → collections.OrderedDict`（`dict` 子类，`dict(...)`/`clear`/`update`/`in`/`[]` 全部兼容，`test_stream.py:231-262` 不受影响） |
...
pool:                         # 成员账：code -> 最近触碰时刻
  上限: policy['pool_max']     # quote/fundflow/timeline/f10/announcement = 2000 (= MAX_DEDUP_CODES)
  淘汰: 超限时删最久未访问者，**仅 del pool[code]** —— 不再 cache.pop / cache_ts.pop
cache:                        # 数据 LRU + TTL
  上限: policy['cache_max']    # quote/fundflow/timeline=2000；f10/announcement=500
  淘汰: 写入超限时 cache.popitem(last=False) + cache_ts.pop(victim)（O(1)）
...
_prefetch_cursor:             # dict[str, int]
  ...
  f10: 0
  announcement: 12
  stream_refresh: 56          # ← stream._refresh_pool 复用的游标键（跨模块约定）
_prefetch_cursor_lock: threading.Lock        # 既有，保留

_PREFETCH_PASS_BUDGET: 60.0                  # 单轮 prefetch 时间预算（保留）
BATCH_MAX_WORKERS: 8                         # ★ 新增公开别名（stream 计算 coverage 用）
_BATCH_MAX_WORKERS: 8                        # 内部别名保留（兼容）
...
```yaml
quote:        {ttl: 8|120,   pool_max: 2000, cache_max: 2000}   # basic_info（+ stock/data 仅用 ttl）
fundflow:     {ttl: 8|120,   pool_max: 2000, cache_max: 2000}
timeline:     {ttl: 8|120,   pool_max: 2000, cache_max: 2000}
...
| 编号 | 规则 |
|------|------|
| **BR-SA-13** | 池是**跨组共享的成员账**（`code → 最近触碰时刻`）；上限 = `policy['pool_max']`（L1/L3 域 = `MAX_DEDUP_CODES=2000`），与 `MAX_DEDUP_CODES` 同源。**禁止**把池上限写成 `MAX_CODES_PER_SUB(200)`（单组上限；会在 ≥2 个不重叠组时抖空缓存） |
...
| **BR-SA-17** | **INV-1b（覆盖条件，供 stream 决策）**：`coverage ≈ floor(0.8 × tick × BATCH_MAX_WORKERS / per_fetch) ≈ 170 取数次数/周期`；`coverage_codes = coverage // len(_FIELD_HANDLERS) ≈ 56 码/周期`；条件 `C1: len(_FIELD_HANDLERS) × N_active ≤ coverage ⇔ N_active ≤ 56` 成立时单周期覆盖整池；否则降级分片轮转（**调度在 `stream.py`**，本模块只提供 `BATCH_MAX_WORKERS` 与轮转原语） |
...
| 编号 | 指标 | 口径 |
|------|------|------|
| **BR-SA-26** | `cache_entries{domain}` | 每次终点缓存写入/淘汰后 `set_gauge(len(cache), key=domain)`；`domain` ∈ {quote,fundflow,timeline,f10,announcement}（`sector` 由 `_sector_cache_put` 另行发布） |
...
from .config import (
    MAX_DEDUP_CODES, REQUEST_TIMEOUT, VALID_STOCK_CODE, _MAX_BATCH_SIZE,
    ...
    _STOCK_DETAIL_BASE_URL, _STOCK_DETAIL_HEADERS, _TIMELINE_BASE_URL,
    _TIMELINE_HEADERS, cache_policy, stock_nav_page_names,
)
...
def _call_fetcher(fetcher, code, deadline, ttl):
    """按 4 档关键字尝试调用；**仅调用帧 TypeError** 才降级（P2-④）。
...
def _fetch_rest_json(url, headers, ttl, deadline=None):
    """唯一 REST 取数收口：**期限透传** + 失败分类 + 解码/结构防御 + 自建失败计数。
    ...
    """
    try:
        raw = json.loads(fetch_json(url, headers, ttl=ttl, deadline=deadline))  # ★ 期限透传
    except FetchError:
...
def _fetch_one(fetcher, code, deadline=None, ttl=None):
    ...
    try:
        return _call_fetcher(fetcher, code, deadline, ttl), None      # §5.2
...
def _run_batch(fetcher, codes, deadline=None, ttl=None, concurrent=True):
    ...
    with ThreadPoolExecutor(max_workers=min(BATCH_MAX_WORKERS, len(codes))) as ex:
        futures = {ex.submit(_fetch_one, fetcher, code, deadline, ttl): code for code in codes}
...
def _process_chunk(codes, domain, policy, fetcher, pool=None, cache=None,
                   cache_ts=None, lock=None, deadline=None, after=None, concurrent=True):
        ...
        with lock:
            for canon in canons:
                ...
                if data is not None and now - cache_ts.get(canon, 0) < ttl:
                    ...
                    missing.append(canon)                              # 过期 ⇒ 拒读+回源（BR-SA-3）
    else:
        missing = list(canons)
...
def _handle_cached_batch(codes, domain, policy, fetcher, pool=None, cache=None,
                         ...
                         per_call_timeout=REQUEST_TIMEOUT, after=None, concurrent=True):
    if not codes:
        return {}, {}
    results, errors = {}, {}
...
| 池上限 = `MAX_DEDUP_CODES`、池/缓存解耦（BR-SA-13/14） | **AC-A7**（去重池 ≤2000）/ **AC-E6** |
...
| 5 | 池上限放大 | §2.1/§4.3 给出 `pool_max='dedup'=2000` | fundflow/timeline 500→2000、f10/announcement 300→2000（basic_info 已在 `config.md` 登记） | 域口径 `dedup`；数据内存改由 `cache_max` 独立 LRU 承担，须纳入 AC-S9/AR-8 总账。**登记项⑤：编排层已批准** |
...
| 10 | CDP 批量预算（**REV-DES-21**） | SAD 未钉数值；**AC-E2 的 ≤15s 仅限 REST 回源**，AC-S7 超时矩阵未含批量整体上限 | `_BATCH_BUDGET_CDP=60`（保留现状） | **编排层已裁决：接受现状 + 登记风险**（f10 非热路径，无 AC 硬违反）。**引据更正为 AC-E2**（REV-DES-17）；**遗留风险**：`/stock/f10` 多码请求端到端可达 60s；缓解 = 建议 `server.md` 强制"f10 仅单码/少量码"（**当前未强制**，编排层同步）+ stream 分片。**登记项⑥** |
...
| 20 | **P7b · `_basic_sector_cache`** | SAD 未提 | 新增 detail 派生行业名缓存（7d `sector` 策略；空值不缓存；无专属 gauge） | BUG-P6C-01 根因 2：quote 域每次 tick 的第二次 REST（行业名）被缓存吸收，稳态回到 1 调用/码 |
...
- [x] 测试要点 29 条（SA-T1..T29；v1.1 新增 T25..T29）映射 PRD AC；AC 追溯矩阵覆盖 A3/A4/A6/A7/A9/A10/E2/E5/E6/S1/S4/S6/S7/S10
