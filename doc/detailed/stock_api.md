...
| 28 | **★ v1.11 · `timeline` 合成极端负载容量边界（AC-E2）** | PRD AC-E2「任何单请求 ≤ 15s」 | **登记为已知容量边界（合成极端负载）**——`/stock/timeline` 在**合成 tier1000**（20 并发 × 50 码）实测 **avg 12.1s / max 24.6s**，超 AC-E2 上界。**归因：上游 / CPU 绑定**（单码 ~96KB JSON 解析成本随并发线性放大），**非连接池**（`HTTP_POOL_MAX_PER_HOST` 24→48 无改善）。**用户裁决（2026-09-21 · B-7）**：选③ = **登记为已知容量边界（合成极端负载）**、**不改码**；真实/常规负载未违反 AC-E2，仅合成「小码数高并发」极端场景暴露。AC-E2 文本与阈值**不变** | 极端合成负载 ≠ 产品缺陷：瓶颈在上游 / CPU 而非连接数（24→48 连接池无改善证明）；现有 `_BATCH_BUDGET_CDP` / 分片轮转装饰性质不同。**触发条件**（若将来常规负载即达此级）需为 timeline 设**并发上限**或下调 `_MAX_BATCH_SIZE`（届时另立 change-set） |
...
- [x] **v1.11（B-5 交叉核对 + B-7 timeline 容量边界登记）**：**BR-SA-26** / **§10#28** —— metrics.md **v1.12** 把 `cache_entries` 标签枚举改为 **`url/feed/quote/depth/fundflow/timeline/f10/announcement/sector/margin`**（删 `longhu`、补 `depth`），**交叉核对结论：本模块 BR-SA-26 无需改动**（`domain ∈ {quote,depth,fundflow,timeline,f10,announcement}` 已含 `depth`、无 `longhu`/`basic_info`，`basic_info` 复用 `quote` 键，`longhu` 为 URL-cache-only 无独立发布）；**§10 新增 #28 —— `/stock/timeline` 合成 tier1000（20 并发 × 50 码）实测 avg 12.1s / max 24.6s 超 PRD AC-E2「≤ 15s」**，归因上游 / CPU 绑定（~96KB JSON/码）、非连接池（`HTTP_POOL_MAX_PER_HOST` 24→48 无改善），**用户裁决（B-7）③ = 登记为已知容量边界（合成极端负载）、不改码**，AC-E2 文本与阈值不变。**本模块代码零改动 / 无行为·接口·对外契约·BR 编号·测试编号变更**；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`metrics.md` → v1.12、`_PROGRESS.md` 同步（B-5/B-7 闭环）**
...
| v1.11 | 2026-09-21 | **B-5 交叉核对 + B-7 timeline 容量边界登记（只改文档）**：metrics.md **v1.12** `cache_entries` 标签枚举（删 `longhu`、补 `depth`）→ **本模块 BR-SA-26 交叉核对一致、无需改动**（`domain` 已含 `depth`、无 `longhu`/`basic_info`）；**§10 新增 #28**：`/stock/timeline` 合成 tier1000（20 并发 × 50 码）实测 **avg 12.1s / max 24.6s 超 PRD AC-E2「≤ 15s」**，归因上游 / CPU 绑定（~96KB JSON/码）、非连接池（24→48 连接池无改善）⇒ **用户裁决（B-7）③登记为已知容量边界（合成极端负载）、不改码**，AC-E2 文本与阈值不变。**本模块代码零改动 / 无行为·接口·对外契约·BR 编号·测试编号变更**；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`metrics.md` → v1.12、`_PROGRESS.md` 同步（B-5/B-7 闭环）** |
