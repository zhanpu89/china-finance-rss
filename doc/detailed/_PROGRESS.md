...
- **本次（B-5/B-6/B-7/B-8 文档闭环 · 2026-09-21 · 只改文档）**：`metrics.md`（→ **v1.12**，B-5 已含）→ 以 **B-5** 完成枚举修正；本轮把 B 类未决项逐项闭环：`stock_api.md` → **v1.11**（B-5 交叉核对 + B-7 §10#28）；`market_api.md` → **v1.10**（B-6 MKT-T20 spy 表述）；`doc/prd/perf-stability-optimization.md` → **v0.11**（B-8 AC-E4 判据窗口校准）+ 本文件状态更新。**B-5 ✅**（metrics.md §3.2 `cache_entries` **补 `depth` 删 `longhu`**；`stock_api.md` BR-SA-26 **交叉核对已正确、无需改枚举**）；**B-6 ✅**（CR-07 命中分支断言改 surrogate spy 计数、可证伪，**526 用例全绿**、业务代码零改动；CR-01/CR-06 仅登记技术债）；**B-7 ✅**（用户裁决③ = **登记为已知容量边界（合成极端负载）、不改码**——`stock_api.md §10#28`；AC-E2 文本/阈值不变）；**B-8 ✅**（用户裁决择一候选不降阈值——AC-E4 基线窗口后移 `[60,180]`vs`[180,300]`、**阈值 20% 不变**、悖论记录，诊断口径 5/5 ≤1.0）。**只改 `doc/detailed/` + `doc/prd/`**（本轮编排层获授权代改 PRD）；**未改代码 / 测试 / `doc/arch/` / `API.md` / `README.md` / `.opencode`。**
- **本次（D 类台账收口 · 2026-09-21 · 只改本文件）**：把 D 类未决项逐项核实、收口——**D-1 ✅**（`API.md` 实际复核：`http_304_total` 显式落点 **`API.md:509`**、HTTP/1.0 304 EOF **`API.md:33`**、四→六字段属内部形态变更非 API 契约面；SAD §9.12 二-1「无显式落点」系过时快照，**无需改 API.md/SAD**）；**D-2 ✅**（口径统一：确定性用例已闭环——STREAM-T12/T13（C1）、CFG-T1/T2/T4/T12（TTL/分层）、T-CACHE-32..36（A8/A3 同源）、SRV-T11（max-age=feed 域）；SAD §8 A3/A8 ⚠️=「实测/校准段待 P6c」，**无需补测试**）；**D-3 ✅**（变更日志登记完成——落 SAD §9.x 修订依据块 + 本文件顶部登记块，本轮为 D 类登记块）；**D-4 ✅**（复核关闭：SAD §7.1 Q2 已裁决 B/B/A′ 且 `API.md:499-511` 同步，PRD §7.2 处置列已含实现后分类、文本无需改）。**D-5 ⬜ 保留**（盘中是否跳过 CDP 守护重启，待用户/编排层决策）。**只改本文件；未改代码 / 测试 / `doc/arch/` / `doc/prd/` / `API.md` / `README.md` / `.opencode`。** 详见下方 D-1..D-5 行。
...
| **B-5** ✅ | 改进建议（标签枚举漂移） | **✅ 已闭环（2026-09-21）**：`metrics.md` → **v1.12** —— §3.2 `cache_entries` 标签枚举**补 `depth`、删 `longhu`**，现行 10 键 = `url/feed/quote/depth/fundflow/timeline/f10/announcement/sector/margin`（`stock_api._cache_store` 以 `key='depth'` 发布；`longhu` 属 URL-cache-only 域、无独立 `key='longhu'` 发布，`basic_info` 复用 `quote` 键）；**`stock_api.md` → v1.11 交叉核对**：**BR-SA-26（:494）已正确、无需改动**（域集合 `{quote,depth,fundflow,timeline,f10,announcement}` 已与现码一致） | 复核后已补 `depth`、裁定 `longhu` 去留 | `metrics.md` §3.2 / `stock_api.md`（BR-SA-26 · §10#28） | task-decomposer |
| **B-6** ✅ | 评审 P2 收尾（PRF-LAT-02） | **✅ 已闭环（2026-09-21）**：**CR-07 已修复**——`tests/test_data_layer.py::test_terminal_cache_publishes_cache_entries_gauge` 命中分支改 **surrogate spy 计数**（包装 `metrics.set_gauge` 收集发布次数，写入后 `publishes.clear()` 隔离命中路径、命中后断言恰 1 次；删命中路径 `set_gauge` **即红**），**526 用例全绿**、业务代码零改动；`market_api.md` → **v1.10** MKT-T20 同步 spy 表述；**CR-01 / CR-06 仅登记技术债、不实施**（margin 终端缓存命中返回共享可变对象当前无触发者；终端缓存原语第 3 处重复因层隔离不宜上收 `cache.py`） | 决定收尾时（CR-06 仅登记、不实施） | `market_api.md` §10 / `tests/` | code-developer + code-reviewer |
| **B-7** ✅ | 已知容量边界（AC-E2 合成高并发尾延迟） | **✅ 已裁定（2026-09-21 · 用户裁决③）**：登记为**已知容量边界（合成极端负载）**、**不改码**——`stock_api.md` → **v1.11 §10 新增 #28**（`/stock/timeline` 合成 tier1000 avg 12.1s / max 24.6s > AC-E2「≤15s」；归因上游 / CPU 绑定 ~96KB JSON/码、**非连接池**（24→48 无改善））；AC-E2 文本与阈值**不变**。**留痕：极端合成负载 ≠ 产品缺陷，常规负载下未违反；勿再作为未闭环上报** | 编排层决策后（选 ①② 需改码） | `stock_api.md` §10#28 / PRD AC-E2 / SAD §4.2 | 编排层决策 + code-developer（若选 ①②） |
| **B-8** ✅ | 判据定义澄清（PRD 级） | **✅ 已修订（2026-09-21）**：PRD → **v0.11** —— AC-E4（§3 `doc/prd/perf-stability-optimization.md:81`）比较窗口由「后 3min vs 前 2min」改为「**对照 `[180,300]`s vs 基线 `[60,180]`s**」（基线后移剔除前 60s 冷启动瞬态；**阈值 20% 不变**、不降阈值铁律），**判据悖论已记录**（A 窗覆盖启动瞬态 ⇒ 判据实度「瞬态多重」、瞬态越轻越易 FAIL），依据 `doc/tester/AC-E4_尖峰因果与判据稳定性.md`（诊断口径 5/5 ≤ 1.0：0.2486/0.8814/0.9728/0.9708/0.4834；候选②中位数 / ③分桶留作后续）。§9.1 新增 v0.11 修订摘要；**AC 总数 36 不变** | 需 PRD 修订（本轮获授权已完成） | PRD §7 / §9 / §9.1 v0.11 + `doc/prd/perf-stability-optimization.md:81` | prd-writer + review-expert |
...
| 标识 | 类型 | 为什么还没做（阻塞原因） | 触发 / 前置条件 | 承接地 | 建议承接方 |
|------|------|------------------------|------------------|--------|-----------|
| **D-1** ✅ | 契约同步（**两份台账冲突**） | **✅ 已裁决（2026-09-21 · 以 `API.md` 实际内容复核）**：SAD §9.12 二-1「无显式落点」系**过时快照**——① `http_304_total` **显式具名落点 = `API.md:509`**（「RSS 条件请求命中 304 的单调计数——停滞即说明 304 未生效」），**非**仅 §2 记的 :507 泛句；② HTTP/1.0 304 EOF **显式落点 = `API.md:33`**（「304 不带 `Content-Length`…`HTTP/1.0` + 每响应关闭连接作答，客户端读到 EOF 即完成」）；③ feed 条目**四→六字段形态变更**按 **SAD N4(c) 自身分类属"内部形态变更、须记入变更日志并同步相关脚本/测试桩"**——**非 API.md 契约面**，对外面（`<ttl>` 元素）已落 `API.md:769`/`:24-34`。⇒ 三者均**已闭环**；「★ 最终收口」§2 结论正确（仅落点引用可更新为 `API.md:509`）。**无需改 API.md / SAD** | **✅ 已复核关闭**（无需文本改动） | SAD §9.12 二-1·§7.1 N4 / `API.md:33`·`:509`·`:769` / 本文件「★ 最终收口」§2 | 编排层（已） |
| **D-2** ✅ | 测试落地（SAD §8 ⚠️） | **✅ 已裁决（2026-09-21 · 口径统一）**：确定性用例**已构成闭环**，SAD §8 的 A3/A8 ⚠️ 与其**不冲突**——⚠️ 语义 =「该 AC 的**实测/校准段**待 P6c」（SAD §9.12 二-4：A3「条件 C1 实测」、A8「TTL 口径测试同步」），非机制未落地。**确定性覆盖实据**：C1/降级 → `stream.md` STREAM-T12/T13（`_trading_tiers` 打桩 + `stream_refresh_lag_ticks` 断言，`tests/test_stream.py:255/262/384/1123`）；TTL/分层 → `config.md` CFG-T1/T2/T4/T12（`config.md:619` 休市日）+ SRV-T11（RSS max-age=feed 域，`tests/test_server_http.py:410`）；A8/A3 同源 → cache.md T-CACHE-32..36（FeedEntryAccessorTests，`tests/test_cache.py:258/312/359/472`）。**526 用例全绿**（既有基线，本轮未增删用例）。⇒ **无需补测试**；残留仅 P6c 实测/校准项（归 A-1/A-4/A-6 类，非 D 类） | **✅ 已复核关闭** | SAD §8（A3/A8）·§2.1 INV-1b / PRD AC-A3·AC-A8·§9.1 v0.6 / `tests/test_stream.py`·`test_config.py`·`test_server_http.py`·`test_cache.py` | 编排层（已）+ tester |
| **D-3** ✅ | 契约同步（变更日志登记） | **✅ 已登记（2026-09-21）**：变更日志 = **SAD §9.x 修订依据块 + 本文件顶部「本次」登记块**（无独立 CHANGELOG 文件，历史同口径）。① Q2 契约同步 → 已记 **SAD §7.1 Q2 / §9.12 行 1**（v1.12）+ `server.md` §2.9.1/§10#7（v1.15 已同步 `API.md:499-511`）；② S4 状态（⚠️→✅）→ 已记 **SAD §8 覆盖声明**（v1.12：`API.md:499-511` 已列三处取值）+ §9.12 行 2；③ AC 落号对齐（F2→A14 / F8→S12）→ 已记 **SAD §8/v1.11** + PRD §9.1 v0.8/v0.9；④ SAD 溯源收口 + 引用时点约定 → 已记 **SAD §9.13**（v1.13）+ 各详设头部。本轮登记块见本文件顶部 | **✅ 已登记关闭** | SAD §9.12 二-6 · §9.13 三-1 · 各登记落点（§9.x 块 + 本文件顶部） | 编排层（已） |
| **D-4** ✅ | 待裁决（PRD 既有漂移） | **✅ 已复核关闭（2026-09-21）**：SAD **§7.1 Q2 已裁决 B/B/A′**（`SAD.md:1270`「Q2 全链路已闭环——代码侧 + `API.md`（`:499-511` 明文取值；`:244` `/stock/basic_info` 已去 ⚡；`:226` `/stock/f10` 保留 ⚡）」）；PRD §7.2 三行**处置列已含实现后分类**（`:411` B 类 / `:412` 已在 AC-S4 改判 B 类 / `:413` A′），与 SAD Q2 **一致**；`API.md:513` 现行取值表与三行吻合。⇒ **无需改动 PRD 文本**（§7.2 = 历史漂移登记块，「待编排层裁决」系登记时点快照，裁决结果已由 SAD Q2 + 处置列承载） | **✅ 已复核关闭** | PRD §7.2 :411-413 / SAD §7.1 Q2·`SAD.md:1270` / `API.md:499-511` | 编排层（已复核） |
