...
> **版本** v1.12 · **状态** 已契约同步（★ v1.12：**`cache_entries` 标签枚举更正如实（B-5 闭环）**——按代码实测发布键**删 `longhu`、补 `depth`**：实际发布键 = `{url,feed,margin,sector}`（直写）+ `_cache_store(key=domain)` 的 `{quote,depth,fundflow,timeline,f10,announcement}`（`stock_api._DOMAIN_STORES` 六域；`basic_info` 走 `quote` 键）；`longhu` 无任何 `key='longhu'` 发布（URL-cache-only 域，恒走 `url` 键，`config.py` cache_max='n/a'）；§10#17 登记。**注册名集合（20 名）/ 签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（标签值不由 `_KNOWN`/`_DEFAULTS` 冻结，`metrics.py` 零代码变更）；★ v1.11：**`cache_entries` 标签枚举补 `margin`**——§3.2 的 `cache_entries` 标签（`key=`）集合补 **`margin`**（`market_api` 终端缓存自 PRF-LAT-02 / v1.8 起在**命中与写入/淘汰均锁外**发布 `set_gauge('cache_entries', len(_margin_cache), key='margin')`，P2 收尾 / CR-02；§10#16 / `market_api.md` BR-MKT-17）；**注册名集合（20 名）/ 签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（`metrics.py` 零代码变更）；★ v1.10：**`upstream_fetch_total` 语义明确 + margin 修复前后差异**——`upstream_fetch_total{domain}` 明确为「**该域 fetch 路径被调用次数（终端缓存 miss 计一次）**」，替换此前"取数点调用 / URL 缓存命中亦计"的上界口径；并补记 **PRF-LAT-02 margin 修复前后差异**（修复前按 handler 调用自增、**高估回源率**；修复后仅终端缓存 miss +1）；**注册名集合（20 名）/ 签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**；★ v1.9：**溯源收口 + 引用时点约定**——上游 **SAD v1.12 / PRD v0.10** 复核无变化，本文为**基础层权威文档**，**无 BR 语义变更、无对外契约变更**；★ v1.8：溯源更正——上游 **SAD v1.12 / PRD v0.10**，**无 BR 语义变更、无对外契约变更**；★ v1.7：§3.2 `upstream_fail_total` owner 列补 **`market_api`**（REV-DES-20 闭环）；★ v1.6：溯源更正 SAD v1.11 / PRD v0.9（末处滞后闭环）；P8 F3：注册 `http_304_total`；P7b 传输层批次：以 `china_finance_rss/metrics.py` + `stream.py` 计数点实现为准；★ **P8 F8 顺带：修正"`metrics.py` 零代码变更"自检句 + 溯源版本更新**）· **日期** 2026-09-18 · **作者/产出** task-decomposer
> **v1.12 变更（`cache_entries` 标签枚举更正如实 · **B-5 闭环** · 只改文档，不改代码）**：① **枚举更正**（§3.2）：`cache_entries` 的标签（`key=`）集合由 `url/feed/quote/fundflow/timeline/f10/announcement/longhu/sector/margin` **删 `longhu`、补 `depth`** → `url/feed/quote/depth/fundflow/timeline/f10/announcement/sector/margin`。依据（**代码实测全链路**）：实际发布键 = **直写四键** `url`（`cache._publish_url_stats` 默认 `metric_key='url'`，cache.py:69）/ `feed`（cache.py:1061）/ `margin`（`market_api` 终端缓存，market_api.py:96/:127）/ `sector`（stock_api.py:773）+ **`_cache_store(key=domain)` 六键** `quote/depth/fundflow/timeline/f10/announcement`（`stock_api._DOMAIN_STORES`，stock_api.py:140-147；`basic_info` 复用 `quote` 键，无独立 `basic_info`）。**`longhu` 无任何 `key='longhu'` 发布**：属 URL-cache-only 域（`config.py` cache_max='n/a'，恒走共享 URL 缓存 ⇒ 计入 `url` 键），§3.2 此前多列。**`depth` 此前漏列**：`_basic_depth_cache`/`depth_prefab` 经 `_cache_store(..., 'depth', now)` 发布（stock_api.py:1124），与 `DOMAIN_MATRIX` 12 域中拥有终端缓存者一一对应。② **为什么是纯文档变更**：同 v1.11——`cache_entries` 是**字典型 gauge**，`set_gauge` 只守卫**指标名**与白名单、**不校验标签值** ⇒ 标签值集合不由注册表冻结（标签值是域名，随 `DOMAIN_MATRIX` 中**拥有终端缓存/feed 缓存**的域演进）。③ **§10#17 登记**。**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（`metrics.py` **零代码变更**）。**本轮只改文档**——代码 / 测试零改动（`stock_api` 域集自 v1.3 起即以 `_cache_store` 发布 `depth`）；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`stock_api.md` → 参照 BR-SA-26（交叉核对见该文档）、`_PROGRESS.md` 同步。**
> **v1.11 变更（`cache_entries` 标签枚举补 `margin` · 只改文档，不改代码）**：① **枚举补登**（§3.2）：`cache_entries` 的标签（`key=`）集合由 `url/feed/quote/fundflow/timeline/f10/announcement/longhu/sector` **补 `margin`**。依据：`market_api` 自 PRF-LAT-02（v1.8）起持有进程内**终端缓存** `_margin_cache`，并在**命中路径与写入/淘汰路径**均**锁外**发布 `metrics.set_gauge('cache_entries', len(_margin_cache), key='margin')`（P2 收尾 / CR-02；`market_api.md` BR-MKT-17）——即 `cache_entries{margin}` 早已是**生产写入的标签值**，此前 §3.2 枚举漏登。② **为什么是纯文档变更**：`cache_entries` 是**字典型 gauge**（`metrics._LABELED_GAUGES`），`set_gauge` 只对**指标名**与白名单做形状守卫、**不校验标签值** ⇒ 标签值集合**不由 `_KNOWN`/`_DEFAULTS` 冻结**（二者只冻结**指标名**），实现无需改动。③ **§10#16 登记**。**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（`metrics.py` **零代码变更**）。**本轮只改文档**——`market_api` 代码 / 测试已落地（**526 用例全绿**）；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`market_api.md` → v1.9、`_PROGRESS.md` 同步。**
> **v1.10 变更（`upstream_fetch_total` 语义明确 + margin 修复前后差异 · 只改文档，不改代码）**：① **语义明确**（§3.2）：`upstream_fetch_total{domain}` = **"该域 fetch 路径被调用次数"**，即该域**终端缓存 miss** 时才 `+1`（`domain ∈ DOMAIN_MATRIX` 键）。此前文档口径（v1.2–v1.9）为"各域取数点调用 / URL 缓存命中亦计数（上界估计）"，在引入终端缓存后**不再准确**。② **margin 修复前后差异（PRF-LAT-02）**：**修复前**（`market_api` ≤ v1.7）`fetch_margin` 在**每次 handler 调用**即 `metrics.incr('upstream_fetch_total', key='margin')`，故 URL 缓存命中也被计入 ⇒ **高估 margin 回源率**；**修复后**（`market_api` v1.8）仅**终端缓存 miss** 时 `+1`（命中路径零计数），与 stock 域（终端 miss 才调 fetcher → 计数，`stock_api.md` BR-SA-28）**对齐**。⇒ 同一流量下 `upstream_fetch_total{margin}` 会**下降**（命中不再计入）——**这是口径更正、非流量变化**，读数时勿误判为上游回源减少。③ **注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（`metrics.py` 代码零变更——本域"是否计一次"由 `market_api` 侧计数点语义决定，本模块只登记名称与零值）。**本轮只改文档**——`market_api` 代码 / 测试已落地（**525 用例全绿**）；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`config.md` → v1.14、`market_api.md` → v1.8、`_PROGRESS.md` 同步。**
> **v1.9 变更（溯源收口 + 引用时点约定 · 只改文档，不改代码）**：① **【溯源收口】** 头部上游 **SAD v1.12 / PRD v0.10** 复核确认（v1.8 已对，无变化）。② **【引用时点约定（新增）】** 本文为**基础层权威文档**——本文件版本**以本文件头部为准**；引用本文的各详设（`stock_api` / `cdp_engine` / `market_api` / `stream` / `server`）其「接口权威」栏所列本文版本 = **其最后一次同步时点的快照**，被引文档（含本文）的**权威版本以各自头部为准**——故该栏**落后一版不属漂移、无需每次追平**；**内容以本文件为准，该栏仅用于定位**。**`metrics.py` 的函数签名 / 注册名集合（20 名）/ 锁 / 快照口径 / `http_304_total` 计数点零变更**（v1.8 正文逐字保留）；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`；`_PROGRESS.md` 同步。**
> **v1.8 变更（溯源更正 · 只改文档，不改代码）**：仅更正头部**溯源**——上游 **SAD v1.11 → v1.12**（依据 `doc/arch/SAD.md` 头部 `**版本** v1.12`，AC 总数 36）、**PRD v0.9 → v0.10**（依据 `doc/prd/perf-stability-optimization.md` 头部 **v0.10**）。**溯源更正，无内容变更**：`metrics.py` 的函数签名 / 注册名集合（20 名）/ 锁 / 快照口径 / `http_304_total` 计数点**零变更**（v1.7 正文逐字保留）；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`；`_PROGRESS.md` 同步。**
> **v1.7 变更（§3.2 owner 列补 `market_api` · 只改文档，不改代码）**：**REV-DES-20 闭环**——`upstream_fail_total` 的 owner 列由「`cache.fetch_json` / `stock_api`」补为「`cache.fetch_json` / `stock_api` / **`market_api`**」（§3.2）。事实依据：`market_api.py` 在**本模块自行判定**的语义失败处写入该计数——`metrics.incr('upstream_fail_total', key='upstream_error')`（`market_api.py:72` JSON 解码 / 未知异常；`:78` `status_code != 0`；`:145` handler 兜底），与 BR-MKT-8「网络失败归 cache、语义失败归 API 层」的防重复 owner 约定一致。**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（v1.6 正文逐字保留）。**未改代码 / SAD / PRD / API.md / README.md / `.opencode`；`market_api.md` → v1.5、`_PROGRESS.md` 同步。**
...
> 上游 SAD `doc/arch/SAD.md` **v1.12**（★ v1.5 溯源更新：原误记 v1.2；★ v1.6 溯源更新：v1.9 → v1.11；★ **v1.8 溯源更新：v1.11 → v1.12**——权威以 SAD 头部为准）（§2.6 / §2.2 R-4 / §4.2 / ADR-010）
> 上游 PRD `doc/prd/perf-stability-optimization.md` **v0.10**（★ v1.5 溯源更新：原误记 v0.3；★ v1.6 溯源更新：v0.8 → v0.9；★ **v1.8 溯源更新：v0.9 → v0.10**）（AC-S10 主；承载 E5/E7/S3/S4/S5/S6/S8/S9 的观测项）
...
- `snapshot()` 为**总函数**且 **`json.dumps` 可序列化**（AC-S10 经 `/healthz` 输出）；**键集合 ⊇ `_KNOWN`**（★ v1.4：**20** 名恒出现，P7b）。
...
_KNOWN: frozenset[str]                     # §3.2 冻结注册表（★ v1.4：20 名），仅供告警
_DEFAULTS: dict[str, Any]                  # ★ P7b：每个注册名的零/空值兜底（★ v1.4：20 键）
...
| `http_304_total` | counter | — | `server._send_not_modified`（**单点**） | ★ v1.4 / F3：304 计数（S10 / 本专项 R1 回归报警；单调、无分母） |
...
| `cache_entries` | gauge(dict) | `url`/`feed`/`quote`/`depth`/`fundflow`/`timeline`/`f10`/`announcement`/`sector`/`margin`（★ v1.11 补 `margin`；★ **v1.12 删 `longhu` 补 `depth`**） | 各缓存写入点（★ v1.11：`margin` = `market_api._margin_cache`，**命中与写入/淘汰均锁外发布**，`market_api.md` BR-MKT-17；★ v1.12：`depth` = `stock_api._basic_depth_cache`，`quote/fundflow/timeline/f10/announcement` = `stock_api._DOMAIN_STORES` 各域，均经 `_cache_store` BR-SA-26 发布；`longhu` 为 URL-cache-only 域恒走 `url` 键、无独立标签） | S10/E6 |
...
| `upstream_fail_total` | counter | `upstream_timeout`/`upstream_error`/`cdp_unavailable` | `cache.fetch_json` / `stock_api` / **`market_api`**（★ v1.7 / REV-DES-20：`market_api` 的语义失败写入点 `market_api.py:72/78/145`） | S10/S3 |
| `upstream_fetch_total` | counter | 域名（`quote`/`depth`/`fundflow`/`timeline`/`plate`/`feed`/`announcement`/`longhu`/`margin`/`f10`/`sector`/`news_url`） | 各域取数点（★ **v1.10 语义：该域 fetch 路径被调用次数 = 终端缓存 miss 才 +1**） | AR-6/Q3⑤ |
...
> **`upstream_fetch_total{domain}` 语义（★ v1.10 明确）**：**该域 fetch 路径被调用次数**——即该域**终端缓存 miss** 时才 `+1`（命中路径不计）。**统一域口径**：`stock_api` 域（`quote`/`depth`/`fundflow`/`timeline`/`f10`/`announcement`/`sector`）终端 miss 才调 fetcher → 计数（**fetcher 内部**的共享 URL 缓存命中**仍计一次**——`stock_api.md` BR-SA-28 的"URL 缓存命中亦计"上界口径**不变**；本版只统一**终端缓存层**口径）；`plate`/`feed`/`news_url`/`longhu` 经共享 URL 缓存（**无终端缓存**）⇒ 每次 `fetch_json` 调用即计一次；**★ `margin` 自 v1.10 起与 stock 域对齐**（`market_api` v1.8 引入终端缓存，仅 miss +1）。**margin 修复前后差异**：修复前（≤ v1.7）按 **handler 调用**自增、URL 缓存命中亦计入 ⇒ **高估回源率**；修复后仅 miss +1 ⇒ 同流量下该计数会**下降**，属**口径更正、非流量变化**（详见 `market_api.md` §10#4 / BR-MKT-9）。
...
> **注册表的"冻结"语义（★ v1.4 修订）**：注册表此前冻结在 **19 名**；★ **v1.4 / F3 经编排层裁定新增 `http_304_total` ⇒ 20 名**（唯一新增来源，须与 `_KNOWN`/`_DEFAULTS` 同 change-set 同步）。除该次裁定外，注册表**不再新增名称**；本表之外的写入会被 warning（BR-MET-3）。**冻结的运行时强制**：BR-MET-3 仅告警（不阻断业务），"键 ⊆ 注册表"由 **MET-T8(b) 集成断言**在 CI 层强制（见 §8/§10#6）。
...
| **BR-MET-11** | **零值恒定发布**：`snapshot()` 结果包含 `_KNOWN` 全部 **20** 名（★ v1.4 / F3）；缺失者取 `_DEFAULTS[name]` 的克隆。已写入的值**恒优先**（兜底只补缺，不覆盖）。`assert set(_DEFAULTS) == _KNOWN` 在导入期保证"注册表 ↔ 零值表"永不脱节。 |
...
| **BR-MET-14**（★ v1.4 / F3 新增） | **`http_304_total`（304 计数）**：在 `_KNOWN` 与 `_DEFAULTS` **同时注册**（`assert set(_DEFAULTS) == _KNOWN` 导入期强制两者同步；注册表 19 → **20** 名），零值为 `0`；写入者 = **`server._send_not_modified`，单点计数**（与 `http_503_total` 的"单点"风格一致 ⇒ 任何 304 都被计入、不可能漏记）。**为什么只需要计数、不需要分母**：该计数**单调**；若"`pubDate` 抖动 ⇒ 永远 200"的回归发生，计数**停止增长**本身就是报警信号（一个既不增长、又无分母的计数即足够），无需 200 分母。`snapshot()` 在从未发生 304 时返回 `0`（零值恒定发布，BUG-P6C-04 风格）。**owner 在 `server.py`**，本模块只登记名称与零值。 |
---
...
_KNOWN = frozenset({
    'http_503_total', 'http_304_total',              # ★ v1.4 / F3：+http_304_total（BR-MET-14）
    'stream_frame_dropped_total', 'stream_queue_bytes',
...
_DEFAULTS = {
    'http_503_total': 0,
    'http_304_total': 0,                             # ★ v1.4 / F3
    'stream_frame_dropped_total': 0, 'stream_queue_bytes': 0,
...
| `snapshot()` | 总函数，绝不抛；registry 全空 ⇒ 仍返回 `_KNOWN` 全 **20** 名的零值（**不是 `{}`**，BR-MET-11） |
...
3. `snapshot()` 的**锁内部分只有浅拷贝**（BR-MET-10）：`_lock` 下做 O(20) 指针拷贝（dict `.copy()` / list `list()`），**深拷贝在锁外** ⇒ 指标数 ≈ 20，`/healthz` 毫秒级（AC-S8 的 zero-upstream 路径），且一次完整深拷贝不会长时间阻塞业务写入。
...
6. 无后台线程、无定时器、无存储 ⇒ 不增加 AC-S9 的线程/内存总账（仅 ~20 个键 + `_DEFAULTS`/`_LABELED_GAUGES` 两张静态表，恒定）。
...
| **MET-T11** reset | `reset()` 后 `snapshot()` **仍含全部 20 名且为零值**（P7b：不再是 `{}`；★ v1.4：20 名） | 测试隔离 / BR-MET-11 |
...
| **MET-T18** `http_304_total` 注册与零值（★ v1.4 / F3 / BR-MET-14） | ① 全新进程未发生 304 ⇒ `snapshot()['http_304_total'] == 0`（零值恒定发布，**不是键缺失**）；② `incr('http_304_total')` 后为 `1`；③ `'http_304_total' in _KNOWN` **且** `in _DEFAULTS`（`assert set(_DEFAULTS) == _KNOWN` 通过；`set(snapshot()) == _KNOWN` 仍成立）；④ 与 `server._send_not_modified` 的集成断言见 `server.md` `SRV-T66` | S10 / **F3** / 本专项 R1 |
...
| **★ v1.4 / F3：`http_304_total`（BR-MET-14 / §3.2 / MET-T18）** | **AC-S10**（304 可观测）/ 本专项 R1（"永远 200"回归的报警信号；`server.md` BR-SRV-46 / SRV-T66） |
...
| 10 | **P7b · 零值恒定发布** | SAD §2.6 未定义"未发生"与"未埋点"的区分 | `_DEFAULTS` 兜底；`snapshot()` 恒含 **20** 名（★ v1.4 / F3）；`assert set(_DEFAULTS) == _KNOWN`（BR-MET-11） | 监控需要 `stream_frame_dropped_total == 0` 而非"键缺失"；兜底表**不写入** registry ⇒ 不改变首次 `incr` 定形语义 |
...
| 12 | **v1.3 · `upstream_fetch_total{depth}`** | SAD §2.6 记分板按 `DOMAIN_MATRIX` 定域；v1.2 文档 domain 示例未含 `depth` | `depth` 域新增（`config.md` §3.1，v1.4）⇒ 其 `upstream_fetch_total` 标签合法，注册表"全名断言"（MET-T8b）不新增指标名、仅新增标签值 | 注册表**不新增名称**（v1.3 时仍 19 名；★ v1.4 因 F3 增至 20，见 #14）；标签值是域名 → 随 `DOMAIN_MATRIX` 演进，不需要新注册 |
...
| 14 | **★ v1.4 · F3：新增 `http_304_total`** | SAD §2.6 计分板（19 名）未列该名；`metrics.md` v1.3 宣称"注册表不新增名称" | `_KNOWN` + `_DEFAULTS` 同步新增 1 名（19 → **20**）；`server._send_not_modified` 单点计数（BR-MET-14） | **P8 / P2-6**：304 的量化收益与"pubDate 抖动 ⇒ 永远 200"回归都需要一个单调计数来观测；`http_503_total` 已有先例。该新增属编排层裁定的**契约变更**；SAD 侧注册表若需回填由 system-architect 承接（本 agent 不改 SAD） |

| 15 | **★ v1.10 · `upstream_fetch_total` 语义（终端缓存 miss 计一次）** | SAD §2.6/AR-6 仅说"每域上游取数计数"，未定义与终端缓存的关系 | 明确为**该域 fetch 路径被调用次数 = 终端缓存 miss 才 +1**；`margin` 自 v1.10 起与 stock 域对齐（`market_api` v1.8 / BR-MKT-9） | 上界口径（URL 缓存命中亦计）在终端缓存引入后**不再准确且高估回源率**；`metrics.py` **零代码变更**（"是否计一次"由各域计数点决定，本模块只登记名称与零值）；`market_api.md` §10#4 / `config.md` v1.14 同步 |

| 16 | **★ v1.11 · `cache_entries` 标签枚举补 `margin`** | SAD §2.6 记 `cache_entries{url,feed,quote,…}` 的**名与标签形态**，未穷举标签值 | §3.2 的 `cache_entries` 标签（`key=`）集合补 **`margin`**——`market_api` 自 PRF-LAT-02（v1.8）起持有进程内终端缓存 `_margin_cache`，**命中与写入/淘汰均锁外**发布 `set_gauge('cache_entries', len(_margin_cache), key='margin')`（`market_api.md` BR-MKT-17） | `cache_entries` 是**字典型 gauge**（`_LABELED_GAUGES`），其标签值对应**拥有终端缓存/feed 缓存**的域；`margin` 自 v1.8 起已从 URL-cache-only 域移出（`config.md` v1.14）⇒ 必须登记，否则 §3.2 枚举与实现漂移。**`metrics.py` 零代码变更**：`set_gauge` 只守卫**指标名**与白名单，**不校验标签值**（标签值集合不由 `_KNOWN`/`_DEFAULTS` 冻结） |

| 17 | **★ v1.12 · `cache_entries` 标签枚举更正如实（B-5）** | SAD §2.6 记 `cache_entries{url,feed,quote,…}` 的**名与标签形态**，未穷举标签值 | §3.2 的 `cache_entries` 标签（`key=`）集合**删 `longhu`、补 `depth`**——按代码实测：发布键 = 直写 `url`/`feed`/`margin`/`sector` + `_cache_store(key=domain)` 的 `quote`/`depth`/`fundflow`/`timeline`/`f10`/`announcement`（`stock_api._DOMAIN_STORES`，stock_api.py:140-147；`basic_info` 复用 `quote` 键）；`longhu` 无任何 `key='longhu'` 发布（URL-cache-only 域，恒走 `url` 键，`config.py` cache_max='n/a'） | 拥有**终端缓存/feed 缓存**的域才有独立标签值；`longhu` 不满足、`depth` 满足（`_basic_depth_cache` 经 `_cache_store(...,'depth',...)` 发布，stock_api.py:1124）——此前枚举与实现漂移（多列 `longhu`、漏列 `depth`）即 B-5。**`metrics.py` 零代码变更**（标签值不由注册表冻结） |
...
- [x] **v1.7（§3.2 owner 补登 · REV-DES-20 闭环）**：`upstream_fail_total` owner 列补 **`market_api`**（语义失败写入点 `market_api.py:72/78/145`）；**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`**
- [x] **v1.8（溯源更正）**：头部上游 **`SAD v1.11 → v1.12` / `PRD v0.9 → v0.10`**（AC 总数 36）；**溯源更正，无内容变更**——**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（v1.7 正文逐字保留）；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`**
- [x] **v1.9（溯源收口 + 引用时点约定）**：上游 **`SAD v1.12` / `PRD v0.10`** 复核无变化；本文为**基础层权威文档**（版本以本文件头部为准），新增「引用时点约定」——引用本文的详设其接口权威栏 = 其最后同步时点快照、**落后一版不属漂移**，内容以本文件为准。**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径零变更**；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`**
- [x] **v1.10（`upstream_fetch_total` 语义明确 + margin 修复前后差异）**：§3.2 表 owner 列 + 表后注明确「**该域 fetch 路径被调用次数 = 终端缓存 miss 才 +1**」；补记 **margin 修复前后差异**（修复前按 handler 调用自增、**高估回源率**；修复后仅 miss +1）；§10#15 登记。**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（`metrics.py` 零代码变更）；**本轮只改文档**——`market_api` 代码/测试已落地（**525 用例全绿**）；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`config.md` → v1.14、`market_api.md` → v1.8、`_PROGRESS.md` 同步**
- [x] **v1.11（`cache_entries` 标签枚举补 `margin`）**：§3.2 的 `cache_entries` 标签（`key=`）集合补 **`margin`**（`market_api` 终端缓存自 v1.8 起在**命中与写入/淘汰均锁外**发布，`market_api.md` BR-MKT-17 / P2 收尾 · CR-02）；§10#16 登记。**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（`metrics.py` **零代码变更**——标签值不由 `_KNOWN`/`_DEFAULTS` 冻结）；**本轮只改文档**——`market_api` 代码 / 测试已落地（**526 用例全绿**）；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`market_api.md` → v1.9、`_PROGRESS.md` 同步**
- [x] **v1.12（`cache_entries` 标签枚举更正如实 · B-5 闭环）**：§3.2 的 `cache_entries` 标签（`key=`）集合**删 `longhu`、补 `depth`**（代码实测发布键 = 直写 `url`/`feed`/`margin`/`sector` + `_cache_store(key=domain)` 的 `quote`/`depth`/`fundflow`/`timeline`/`f10`/`announcement`，`stock_api._DOMAIN_STORES`；`longhu` 为 URL-cache-only 域恒走 `url` 键、无独立标签）；§10#17 登记。**注册名集合（20 名）/ 函数签名 / 锁 / 快照口径 / `http_304_total` 计数点零变更**（`metrics.py` **零代码变更**）；**本轮只改文档**——代码 / 测试零改动；**未改代码 / 测试 / SAD / PRD / API.md / README.md / `.opencode`；`stock_api.md` 交叉核对（BR-SA-26）、`_PROGRESS.md` 同步**
