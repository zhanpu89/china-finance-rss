...
| `cache_entries` | gauge(dict) | `url`/`feed`/`quote`/`fundflow`/`timeline`/`f10`/`announcement`/`longhu`/`sector`/`margin`（★ v1.11 补 `margin`） | 各缓存写入点（★ v1.11：`margin` = `market_api._margin_cache`，**命中与写入/淘汰均锁外发布**，`market_api.md` BR-MKT-17） | S10/E6 |
