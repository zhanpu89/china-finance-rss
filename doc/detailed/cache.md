...
> **版本** v1.15 · **状态** 增量设计（★ v1.15 **溯源收口 + 引用时点约定**：上游 SAD v1.12 / PRD v0.10 复核无变化，本文为**基础层权威文档**，**无 BR 语义变更、无对外契约变更**；★ v1.14 溯源更正：上游 SAD v1.12 / PRD v0.10，**溯源更正，无 BR 语义变更、无对外契约变更**；★ v1.13 实现事实对齐：`_feed_degraded_warned`/`_warn_feed_degraded_once` 写入 BR-CACHE-36（**按原因**去重）+ §10#30 由「登记不修」更正为「已防御」（三处 `isinstance` 守卫）+ `_sweep_expired` 跳过非映射条目 + 溯源 SAD v1.11 / PRD v0.9；P3a · `feed_cache_get_entry`；P3a-r1 评审定向修：扩 **T-CACHE-32** 负例 + 头部溯源修正；P7b 漂移回填：T-CACHE-32 标注已实现；**P8 F1/F5：feed 条目 `fingerprint`/`last_modified` + `feed_cache_put` 返回写入条目**；**P8 F8：feed 取数锁表引用计数收敛原语**；**P2-1 fail-safe：legacy 条目缺 `last_modified` ⇒ 读 `None`（禁用 IMS，不抛 `KeyError`）+ 溯源 PRD v0.8**；**读侧 fail-safe 口径补全（缺 `xml` ⇒ 视同 miss；`time`/`expires_at` 以 `.get` 读取）+ `feed_fetch_acquire` 实现约束（先 `get` 判空、未命中才建 `Lock`）**；**最后一轮契约收口：`''` 边界写死 + 残缺降级可观测（`log.warning` 去重）+ 边界登记（§10#29/#30）**）· **日期** 2026-09-18 · **作者/产出** task-decomposer
> **v1.15 变更（溯源收口 + 引用时点约定 · 只改文档，不改代码）**：① **【溯源收口】** 头部上游 **SAD v1.12 / PRD v0.10** 复核确认（v1.14 已对，无变化）。② **【引用时点约定（新增）】** 本文为**基础层权威文档**——本文件版本**以本文件头部为准**；引用本文的各详设（`stock_api` / `market_api` / `stream` / `server`；`cdp_engine` 与本模块无接口）其「接口权威」栏所列本文版本 = **其最后一次同步时点的快照**，被引文档（含本文）的**权威版本以各自头部为准**——故该栏**落后一版不属漂移、无需每次追平**；**内容以本文件为准，该栏仅用于定位**。**`feed_cache_get`/`feed_cache_get_entry`/`feed_cache_put`/`feed_fetch_acquire`/`feed_fetch_release` 签名语义、锁序、TTL/清扫/上限、BR-CACHE-32..36 正文全部不变**（v1.14 正文逐字保留）；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`；`_PROGRESS.md` 同步。**
> **v1.14 变更（溯源更正 · 只改文档，不改代码）**：仅更正头部**溯源**——上游 **SAD v1.11 → v1.12**（依据 `doc/arch/SAD.md` 头部 `**版本** v1.12`，AC 总数 36）、**PRD v0.9 → v0.10**（依据 `doc/prd/perf-stability-optimization.md` 头部 **v0.10**）。**溯源更正，无 BR 语义变更、无对外契约变更**——`feed_cache_get`/`feed_cache_get_entry`/`feed_cache_put`/`feed_fetch_acquire`/`feed_fetch_release` 签名与语义、锁序、TTL/清扫/上限、BR-CACHE-32..36 正文**全部不变**（v1.13 正文逐字保留）；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`；`_PROGRESS.md` 同步。**
...
> 上游 SAD `doc/arch/SAD.md` **v1.12**（★ v1.6 溯源修正：原误记 v1.2；★ v1.9 溯源更新：v1.7 → v1.9；★ v1.13 溯源更新：v1.9 → v1.11；★ **v1.14 溯源更新：v1.11 → v1.12**——权威以 SAD 头部为准）（§2.1 / §2.2 R-3 / §2.3 D-1 D-2 D-5 / §2.4 / ADR-002/003/008/009/014）
> 上游 PRD `doc/prd/perf-stability-optimization.md` **v0.10**（★ v1.6 溯源修正：原误记 v0.3；★ v1.10 溯源更新：v0.6 → v0.8；★ v1.13 溯源更新：v0.8 → v0.9；★ **v1.14 溯源更新：v0.9 → v0.10**——权威以 PRD 头部为准）（AC-S3/S6/A4/A9/A10/E6/E9/A10）
...
- [x] **v1.14（溯源更正）**：头部上游 **SAD v1.11 → v1.12 / PRD v0.9 → v0.10**（AC 总数 36）；**溯源更正，无 BR 语义变更、无对外契约变更**——`feed_cache_get`/`feed_cache_get_entry`/`feed_cache_put`/`feed_fetch_acquire`/`feed_fetch_release` 签名语义、锁序、TTL/清扫/上限、BR-CACHE-32..36 正文**全部不变**（v1.13 正文逐字保留）；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`**
- [x] **v1.15（溯源收口 + 引用时点约定）**：上游 **SAD v1.12 / PRD v0.10** 复核无变化；本文为**基础层权威文档**（版本以本文件头部为准），新增「引用时点约定」——引用本文的详设其接口权威栏 = 其最后同步时点快照、**落后一版不属漂移**，内容以本文件为准。**签名语义 / 锁序 / TTL·清扫·上限 / BR-CACHE-32..36 正文零变更**；**未改代码 / SAD / PRD / API.md / README.md / `.opencode`**
