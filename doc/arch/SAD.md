...
> **文档编号** SAD-2026-P6C-01 · **版本** v1.11 · **状态** 待复审
> **日期** 2026-09-18 · **产出** system-architect
> **上游输入** PRD `doc/prd/perf-stability-optimization.md`（**v0.9，36 AC**（高效 10 / 准确 14 / 稳定 12）/ R1-R20；SAD v1.8 承接的 RSS 条件请求 **AC-A13**、v1.9 承接 **AC-A14**、v1.10 承接 **AC-S12** 由 prd-writer 并行落号，落号对齐见 §8 / §9.11）
...
*SAD v1.11 完 · 供 review-expert 复审（复核 §9.11 AC 落号对齐（F2→AC-A14 / F8→AC-S12 / AC 总数 34→36）、§2.7 `last_modified=None` 第二来源（legacy / 非六字段条目 P2-1 fail-safe）、§8 追溯矩阵 + 覆盖声明，及 §9.10 F8 回填（feed 取数锁表引用计数回收 / 不变式"计数 ≥1 ⇒ 不可 pop" / 有界 = 并发在飞键数 / `BR-SRV-45` 论证边界补正 / `SRV-T71`·`T72`·`T-CACHE-35`）、§9.9/§9.8/§9.7 的历史承接）与 task-decomposer 承接（`server.md` v1.11 / `cache.md` v1.10 / `metrics.md` v1.5 已先行；`API.md` 待同步 RSS `Last-Modified` 语义修订、`http_304_total`、feed 条目六字段形态变更、legacy 条目 `last_modified` fail-safe 与 HTTP/1.0 304 EOF 说明）*
