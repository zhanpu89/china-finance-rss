...
> **文档编号** SAD-2026-P6C-01 · **版本** v1.16 · **状态** 待复审
...
> **修订依据（v1.9）** P8 **对抗性盲审 F1–F7 架构回填**——RSS 条件请求（v1.8 承接）经 P8 盲审暴露 **2×P1 + 8×P2**，编排层裁定修复 **F1–F7**，详设先行升级（`server.md` **v1.9** / `cache.md` **v1.8** / `metrics.md` **v1.4**）且代码已实现；本轮把架构级结论回填 SAD：① **指标注册表 19 → 20**——新增 `http_304_total`（`metrics._KNOWN` + `_DEFAULTS` 同步注册，`server._send_not_modified` **单点**计数），修正全文"19 个度量名 / 注册表冻结不新增"表述与计分板（§2.6 / §3 metrics 行 / 专项仅此一次新增，`metrics.md` v1.4 已把"注册表不新增（SAD 契约冻结）"改写为"除本次裁定外不再新增"）（**F3**）；② **AR-16 修订（F1）**——RSS `Last-Modified` 语义由"缓存写入时刻"改为**表示最后一次变更的时刻**（feed 条目承载 `fingerprint`（与 ETag 同源）与 `last_modified`；指纹未变则跨 TTL 重生成**继承**同一 `last_modified`；不变式 **`ETag` 变 ⟺ `Last-Modified` 前进**）；原"IMS-only 客户端跨 TTL 必得 200"的**取舍已消除**，相关旧表述一律改掉（§2.7 / AR-16）；③ **ADR-023 修订（追加，不重写历史）**——记 F1 选型（否决备选 `Last-Modified = max(items.pubDate)`）与 F2 裁定 + 放大风险论证与接受（§5 ADR-023 / §2.7）；④ **§2.1 D4「承诺=行为」推导**（F4）：RSS `max-age` authority 由 `news_url` 改 **`feed` 域**（今日数值不变 30/180，只换权威域），原"`news_url` 与 `feed` 同 L3"的依赖消失——属**强化**而非矛盾（§2.1 D4）；⑤ **新增内部契约的架构级说明**（feed 缓存键的表示作用域要求；`_feed_ttl_minutes() == ceil(feed 路径 max-age/60)` 一致性不变式）（§2.7）；⑥ **N4 核对更新**（**F6** HTTP/1.0 下 304 无 `Content-Length` 由 EOF 收尾、RFC 9110 §8.6 禁发 `Content-Length: 0`，本轮不升 `protocol_version`，中间件收尾为理论风险；`feed_cache_put` 返回值契约；条目**四→六字段**的**形态变更**——任何手改 feed 条目的旧代码会 `KeyError`）（§7.1 N4）。逐项见 **§9.9**。
> **修订依据（v1.10）** P8 **F8 架构回填（feed 取数锁表有界收敛）**——RSS 条件请求（v1.8 承接、v1.9 回填 F1–F7）经代码评审 `doc/review/rss-conditional-get_p8修复_代码评审_专家版.md`（`CR-RSS-20260918-003`）查出 **P1：F2 引入的 `_feed_fetch_locks` 按含 `base_url` 的键建锁且全仓无淘汰 ⇒ 合法 Host 可枚举、锁表无界增长 + 每请求本地重生成 XML**；编排层裁定修复 **F8**，详设先行升级（`server.md` **v1.10**：BR-SRV-50 新增 + BR-SRV-45 放大论证**补正**；`cache.md` **v1.9**：BR-CACHE-35 + `feed_fetch_acquire`/`feed_fetch_release`/`_feed_fetch_refs`）且代码已实现、**511 用例全绿**。本轮把架构级结论回填 SAD：① **§2.2 新增 R-7「进程内 per-key 资源生命周期」**——feed 取数锁表（`_feed_fetch_locks` + `_feed_fetch_refs`）必须**与键空间同界收敛**（引用计数在 `acquire` 时 +1 且**先于返回锁**、`release` 在 `finally` 递减、**归零两表同删**；不变式"计数 ≥1 ⇒ 该键不可回收"；**为什么不能简单 `pop`**：归零前 `pop` 会让"已取锁但未进 `with`"的线程与后来者拿两把不同的锁 ⇒ 同键双抓 = 历史事故 **TS-4**，见 `doc/review/系统_代码评审报告_001.md`）；**有界性结论 = 锁表规模为"并发在飞的键数"，与"累计见过的键数"无关，`PUBLIC_BASE_URL` 已设时恒为 5 条 path**（**F8**）；② **§2.7 补 F8 有界性结论 + `BR-SRV-45` 论证边界**（原放大论证只覆盖上游请求，未覆盖锁表内存与本地生成；**上游请求不放大 ≠ 本地资源不放大**，后者的界由 F8 提供）；③ §3 cache/server 行 / §4.3 资源行（锁表上界）/ §8 追溯矩阵（**F8 行**：`BR-SRV-50`/`BR-CACHE-35` ↔ `SRV-T71`/`T72`/`T-CACHE-35`）；④ `tech-stack.json` **v1.9 → v1.10**（`backend.cache` + `namingRules.concurrency`：per-key 资源引用计数回收约定）。**只增编号**（新增 R-7；未删任何 BR/AR/ADR/N）。逐项见 **§9.10**。
...
> **端锁定** 🟠 STABLE（仅**新增**下划线保留键与观测字段；`feeds[].status` 属**改既有字段取值**——**已由编排层批准并同步（`API.md:499-511`）**，见 §7.1 Q2）。**v1.4 新增两项边界（须编排层登记）**：① SSE 帧**只增**元数据（`codes_total`/`fields`/`missing`/`missing_count`/`errors`/`stale`/`stale_count`）属 🟠 STABLE 的"只增"；② **JSON 单体/面板/工具端点业务降级维持 200**（v1.5 N1 裁决关闭）——这不是"状态码变更"，而是**保持旧版本契约**，与 PRD AC-A5 逐格一致；真实 503 仅**连接准入拒绝**（主/流端口）与 **`/healthz` degraded**（健康端点自身语义，**不构成"业务端点可 503"的先例**）（§2.4 / §7.1 N1）。**v1.7 新增两项「只增」边界（须编排层登记）**：③ SSE `quote` 字段负载内新增 `depth`（五档盘口）——`items[code].quote` 由"basic 客体"变为"basic 客体 **+ `depth` 键**"，属 🟠 STABLE「只增」（既有键类型与含义不变；空 dict/全 0/非普通股/指数 ⇒ **该键不出现**，不伪造）；④ `POST`/`PATCH /stream/subscriptions` 响应新增 `refresh_capacity_codes`/`refresh_lag_ticks`/`capacity_warning`（只增）。**v1.8 新增「只增」边界（须编排层登记）**：⑤ **RSS 条件请求**——5 个 `ROUTES` feed 的 200 新增 `ETag`（弱校验器）/`Last-Modified` 头，条件命中返回 **`304`**（无 body / 无 `Content-Encoding` / 无 `Content-Length`），feed XML 新增 **`<ttl>`**（advisory，分钟）；**无任何条件头时 200 body/状态码逐字不变**，`Vary`/`private` 反缓存投毒语义与业务降级恒 200 全部保持（§2.7 / §3.1 例外 7 / §7.1 N4）。**v1.9 边界澄清（承接 P8 F1/F2/F4，须编排层登记）**：⑥ RSS 条件请求的**对外头项零新增**——`ETag`/`Last-Modified`/`304`/`<ttl>` 均属 v1.8 已登记「只增」；本轮只**收紧语义**（`Last-Modified` = 表示最后一次变更的时刻，跨 TTL 仍 304 ⇒ 对 IMS-only 客户端是**收益增强**，非新契约面）与**修正内部键作用域**（feed 缓存键含 `base_url`，属 server↔cache 内部契约，cache 层视键为不透明串）；`max-age` authority 由 `news_url` 改 `feed` 域但**数值逐字不变**（盘 30 / 非盘 180）；`http_304_total` 为**新增观测名**（`/healthz.metrics` 只增），§2.7 / §7.1 N4
...
>
> ⚠️ **v1.16 实测更正（PRF-MEM-01 实测回填；取代上一条 v1.14 注末的 `≈0.65GiB` 乐观估算）**：上一条"预期 python 稳态 RSS ≈0.65GiB"**已被重建部署实测（run 20260920-110805）证伪**。**实测（权威）**：收缩前 python 进程 RSS **`1.095GiB`** / 容器 **`1.449GiB = 96.57%`** → 收缩后 python RSS **`1.012GiB`** / 容器 **`1.41GiB = 93.98%`**，**净收益仅 ≈`83MiB`**（3 域配置经 healthz 实测精确生效：quote/fundflow=1000/1000、timeline=500/500）。**归因更正**：终端缓存收缩为**小头**（按单条原始体积反推删除量 ≈92MB，与 −83MiB 吻合）；**大头 = 共享 URL 缓存**（`cache.MAX_CACHE_SIZE=2000` 顶满、条目为**解析后 Python 对象**）与**线程池 glibc arena 碎片**。§4.3 的 **≈278MB 只是「已知缓存项」上界，不是进程总内存上界**（详见 §4.3 口径声明 / §7.2 **AR-18**）。

> ⚠️ **v1.15 更正（CR-02，PRF-MEM-01 修复轮，按代码回写）**：上一条 v1.14 注中的"改用具名 `'fixed:N'`"**已由代码再次变更**——三域 spec 现为 **`'env:<NAME>'`**：`quote` = **`'env:MAX_QUOTE_POOL'`**、`fundflow` = **`'env:MAX_FUNDFLOW_POOL'`**、`timeline` = **`'env:MAX_TIMELINE_POOL'`**（`config.py` `DOMAIN_MATRIX`）。**默认值逐字不变**（1000 / 1000 / 500）⇒ v1.14 的**数值结论全部保留**，v1.15 只把**来源**从"代码内字面量"恢复为"**env 注册常量**"（部署可免改码调参）。`_resolve_pool_max` 在**调用期**经白名单 `_POOL_MAX_ENVS`（`frozenset({'MAX_QUOTE_POOL','MAX_FUNDFLOW_POOL','MAX_TIMELINE_POOL'})`）解析为模块常量，**未知 NAME → `ValueError`**（fail-fast，不留静默回落）。**边界（务必区分）**：`cache_max`（1000/1000/500）**仍是矩阵内的整数字面量、不经 env**——它是**内存硬界**（§4.3），env 只能调**池上限**（成员账规模 / prefetch 轮转覆盖），**调 pool 不改变内存界**。
>
...
- D4：`_get_or_fetch_feed` 的 `expires_at` 由 `cache_policy('feed')['ttl']` 派生（交易时段 = L3=30s，**非交易时段 = L3=180s**），与 `_cache_age()` 的 RSS 分支**读同一个 `feed` 域**，保证"承诺 = 行为"。
  > ⚠️ **v1.9 / F4 推导强化（P8 P2-4）**：SAD 早先的"承诺 = 行为"推导隐含依赖 **`news_url` 与 `feed` 两域当前恰好同 L3**（谁做 authority 数值都一样）；F4 之后 **RSS 响应的 `max-age` authority 显式定为 `feed` 域**（`_CACHE_AGE_DOMAINS` 的 5 个 feed path 由 `news_url` 改映射 `'feed'`）——**该"两域同值"的依赖消失**，`<ttl>`、feed 缓存 TTL、`Cache-Control: max-age` 三者同源单一 authority，属**强化而非矛盾**。原 `news_url` 的语义是"上游 URL 取数缓存"，用它决定 **RSS 响应**的 `max-age` 属语义错配（此前被"两域恰好同 L3"掩盖）。**今日数值逐字不变**（盘 30 / 非盘 180）。一致性不变式见 §2.7（`_feed_ttl_minutes() == ceil(feed 路径 max-age / 60)`）。
...
- **★ v1.16 口径声明（缓存上界 ≠ 进程 RSS）**：R-4 与 §4.3 的**内存上界只覆盖「已知缓存项」**（URL/feed/端点/队列/`_last_known` 等），**不是进程总内存上界**——PRF-MEM-01 实测 python 进程 RSS **`1.012GiB`** 与 §4.3 已知缓存项上界 **≈278MB** 相差 **≈750MB**，缺口**未纳入分项建模**（候选：**共享 URL 缓存**（`MAX_CACHE_SIZE=2000` 顶满、解析后对象）+ **线程池 glibc arena 碎片** + CDP/运行时）。**登记为待解架构项 AR-18**（§7.2）；由此 **「调小 `cache_max` ⇒ 进程内存线性下降」的推断不成立**。
...
**R-7 进程内 per-key 资源生命周期（feed 取数锁表；★ v1.10 / F8 新增）**

`cache._feed_fetch_locks`（`cache_key → threading.Lock`）与 `cache._feed_fetch_refs`（`cache_key → int`）同受 `_feed_fetch_locks_lock`；**唯一读写入口 = `feed_fetch_acquire(cache_key) -> Lock` / `feed_fetch_release(cache_key)`**（`cache.md` BR-CACHE-35），`server._get_or_fetch_feed` **不再直接 import 锁表**（`server.md` BR-SRV-50；§3 cache/server 行）。

- **生命周期（`acquire`/`release` 成对）**：`acquire` = `setdefault` 建锁（必要时）+ 引用计数 **+1**，**计数先于 `return lock`**（覆盖"已取出锁但尚未进入 `with`"的窗口）；`release` = 计数 **−1**，**归零时 `pop` `_feed_fetch_locks[key]` 且 `pop` `_feed_fetch_refs[key]`（两表同删）**，否则写回。调用方必须 `lock = acquire(k)` → `try: … finally: release(k)`——**异常路径（`fetch_func()` 抛 / `feed_cache_put` 抛）也必须释放**，否则计数泄漏 ⇒ 锁表退化为只增不减。
- **不变式（可白盒断言）**：只要还有线程**持有或即将获取**该键的锁，计数 ≥1 ⇒ 该键**不可能**被 `pop`；`pop` 只发生在计数归零（无持有者/等待者）时。
- **为什么不能简单 `pop`**：归零前 `pop` 会让"已取出锁但尚未进入 `with`"的线程与后来者各拿一把**不同的锁** ⇒ **同一键双抓**（历史事故 **TS-4**，`doc/review/系统_代码评审报告_001.md`）。引用计数正是为覆盖"取出到 `with`"这段窗口而存在。
- **正确性论证**：`pop` 只发生在计数归零时；随后到达的线程创建新锁，其**双检仍命中**前一个持有者已写入的缓存条目 ⇒ **不产生重复取数**（第二次真取数只可能因为条目确实已过期）。
- **有界性结论**：锁表规模收敛于"**并发在飞的键数**"，而非"**累计见过的键数**"；`PUBLIC_BASE_URL` 已设时键恒为 **5 条 path**。**可选运营缓解（备注，非契约）**：部署侧强制设 `PUBLIC_BASE_URL`。
- **与 §2.2 其他资源界的关系**：R-3 界缓存条目、R-4 界内存总账、R-6 界刷新集合；R-7 界的是 **per-key 锁/计数的生命周期**——四者合起来才使 AC-S9「24h 资源不单调增长」对"外部可枚举的键空间"成立；且 R-7 直接承接 **AC-S12「feed 取数锁表随并发收敛（per-key 资源有界）」**（主承接编号 = AC-S12，AC-S9 为并列资源总账；§4.3 锁表上界行）。

**不选其他（R-7）**：不重加朴素 `pop`（重演 TS-4 双抓）；不给锁表加 TTL/LRU（需时钟且仍不覆盖"已取锁未进 `with`"窗口）；不引入单一全局锁串行化全部 feed 取数（破坏按键隔离，把并发度压到 1）。
...
| 指标 | 来源 | 对应 |
|------|------|------|
| `http_503_total` | **3 个计数点（v1.5；N1 关闭后由 4 收敛为 3）**：① `BoundedThreadPoolServer._reject_503`（主端口准入拒绝，`server.py`）；② `stream._serve_sse` 连接上限 503（流端口准入拒绝，`stream.py`）；③ `/healthz` 的 503（`status=degraded` 或 guard 失败体）。**业务端点不产生 503**（§2.4 N1） | AC-S10 / S5 |
| **`http_304_total`（★ v1.9 / F3 新增）** | `server._send_not_modified`（**单点**计数，在 `end_headers()` 之前；与 `http_503_total` 的单点风格一致 ⇒ 任何 304 都被计入、不可能漏记）。**注册表 19 → 20 名**（`_KNOWN` + `_DEFAULTS` 同步，`assert set(_DEFAULTS) == _KNOWN` 导入期互锁；`metrics.py` 本身零代码变更，实现改在 `server.py`）。**只需计数、不需分母**：该计数**单调**，若"`pubDate` 抖动 ⇒ 永远 200"回归发生，计数**停止增长**即报警信号 | AC-S10 / 本专项 R1 回归 |
...
- 响应字段（**只增不改**；原"不改既有 `feeds[]` 字段与含义"**删除**，见 Q2）——**精确 schema**：
  ```jsonc
  {
    ...
    "policy":  { "<domain>": {"tier":"L1","ttl":8,"pool_refresh":8,"pool_max":1000,"cache_max":1000}, ... },   // ★ v1.14 PRF-MEM-01：pool_max/cache_max 按域声明（quote/fundflow=1000、timeline=500；depth/announcement/f10='dedup'=2000），此处以 fundflow 为例；★ v1.15 CR-02：pool_max 来源 = 'env:MAX_FUNDFLOW_POOL'（env 注册常量，默认 1000、部署可调），cache_max=1000 仍为字面量（内存硬界）
    "cdp":     { "state":"idle|restarting|unavailable", "window_start":<epoch|null>, "window_end":<epoch|null> }
  }
  ```
  > 修 P2-N1：上一版"精确 schema"漏列既有 `feeds[]` / `cache_ttl` / `request_timeout`，与 🟠 STABLE「只增不改」及 ADR-001「`cache_ttl` 保留字段名与含义」冲突。**既有键一个不少**，仅在其上叠加 `stale`/`metrics`/`policy`/`cdp`；`feeds[].status` 的取值变更已按 Q2 由编排层批准并同步（`API.md:499-511`）。
- **契约口径（Q2）**：`feeds[].status` 的取值由 `requires_chrome_cdp`/标注漂移改为目标值属「**改既有字段取值**」（≠"只增不改"），**已由编排层批准并同步 `API.md:499-511`**（见 §7.1 Q2；变更日志登记为编排层待办）。
...
> ★ **v1.9 / F6 收尾契约补记（P8 P2-7，仅文档化，不改代码）**：HTTP/1.0 下 **304 不得发 `Content-Length: 0`**（RFC 9110 §8.6 明令，若发则其值必须等于该资源 200 的体长——本服务 200 体 ~37–45KB），响应以**连接关闭（EOF）**收尾；**本轮不升 `protocol_version` / keep-alive**（BR-SRV-44 出范围——该变更波及全部端点的连接语义与线程池占用假设）。**权衡如实登记**：中间件（代理/CDN）对"无长度 304 的 EOF 收尾"处理属**理论风险**，`http.client` 已实测通过（`SRV-T47`/`T62` 断言无 `Content-Length`/`Content-Type`/`Content-Encoding` 且无 body）。见 §7.1 **N4**。
...
**`Last-Modified` 时间源（★ v1.9 / F1 改写：表示最后一次变更的时刻）**：`Last-Modified = cache.feed_cache_get_entry(cache_key)['last_modified']`（epoch 秒；发送格式 `formatdate(timeval=…, localtime=False, usegmt=True)`）。**时间源 = feed 条目的 `last_modified` 字段**，由 `feed_cache_put` 维护：写入时与**同一键的上一条目**（★ **即使该条目已过期也参与比较**——这正是"跨 TTL 重生成仍 304"的机制）比较 `fingerprint`（规范化摘要，**与 ETag 同源**）：相同 ⇒ **继承**上一 `last_modified`；不同 ⇒ 取**本次写入时刻**。**不变式（可断言）**：在"同一键存在上一条目"的路径上，**`ETag` 变 ⟺ `Last-Modified` 前进**（二者同源同一 `fingerprint`）；**唯一例外** = 无上一条目（首次 / LRU 淘汰 / 已 sweep）⇒ `last_modified = 本次写入时刻`（退化为一次性 200 后恢复 304，**已登记的可接受降级**）。**仅在真实缓存条目存在且含该字段时**发送——**`last_modified` 为 `None` 有两个来源**：① 降级/回源失败路径（`_guard` rss 分支返回 `(error_xml, None)`）；② ★ **v1.11 补（覆盖缺口，P2-1 fail-safe）legacy / 非六字段缓存条目缺 `last_modified` 字段**——`feed_cache_get_entry` 以 **`entry.get('last_modified')`** 读取（缺省 `None`），故旧形态条目（仅 `xml/time/last_access/expires_at` 四字段）**绝不抛 `KeyError` 经 `_guard` 变成降级体**（降级体是"上游失败"语义，缺字段只是"该条目无变更时刻"，二者必须区分）。两来源口径一致：**不发 `Last-Modified`、禁用 IMS**（`If-Modified-Since` 不可评估），**`If-None-Match` 仍按 ETag 正常评估**；绝不拿陈旧条目时间冒充降级体。详设 `server.md` **v1.11** / `cache.md` **v1.10**。**副作用（语义正确）**：`last_modified` 可能**远早于** body 内 `<lastBuildDate>`（前者表示"未变"，后者是生成时刻）。
> ★ **v1.9 / F1 取消的旧取舍**：v1.8 曾取 `entry['time']`（缓存**写入时刻**）为 `Last-Modified`，其已登记取舍为"**仅带 IMS 的客户端跨 TTL 边界（写入时刻前进）仍得 200 全量**"——**该取舍已随 F1 消除**：指纹未变则 `last_modified` **不前进**，IMS-only 客户端跨 TTL 亦得 **304**（与 INM-only 等价）。**已否决的备选 `Last-Modified = max(items 的 pubDate)`**：免状态，但上游"改描述不改 `pubDate`"会发出**错误 304（陈旧数据）**——对交易数据服务，**错误 304 远比多发 200 严重**，故否决（§5 ADR-023 追加 / §7.2 AR-16）。
...
| **304 组成** | 由专用 `_send_not_modified` 生成：**无 body / 无 `Content-Encoding` / 无 `Content-Length` / 无 `Content-Type`**；**必带** `ETag`（当前弱标签）+ `Last-Modified`（有值时）+ `Cache-Control`（**与 200 同值**，同一 `_cache_age()` 来源）+ `Vary`（`varies_on_host` 时为 Host 三件套 + `Accept-Encoding`，否则 `Accept-Encoding`）。**304 不写 feed 缓存**（LRU 触碰由读取路径完成）。`HEAD` 与 `GET` 走同一条件判定。**★ v1.9 / F3**：`_send_not_modified` 内**单点** `metrics.incr('http_304_total')`（在 `end_headers()` 之前；与 `http_503_total` 同风格 ⇒ 不可能漏记） |
...
**feed 缓存键的表示作用域（★ v1.9 / F2 新增，架构级内部契约）**：**键必须覆盖表示的全部可变维度**——`cache_key = _feed_cache_key(path, base_url)`：`PUBLIC_BASE_URL` **已设** ⇒ `= path`（表示与 Host 无关，与 v1.8 现状逐字一致）；**未设** ⇒ `= path + '\x00' + base_url`（`base_url` = `_base_url()` 结果）。**理由**：feed body 内嵌**请求 Host 派生**的 `<atom:link rel=self>`，而缓存此前**只按 `path` 建键** ⇒ 一条伪造 Host 的请求即可在**一个 TTL 内改写所有读者的订阅链接**，即 **`Vary: Host, X-Forwarded-Host, X-Forwarded-Proto` 声明了什么，源站自身的缓存键就必须覆盖什么**（头与行为一致）。**键作用域归属**：键对 `cache.py` 是**不透明字符串**（cache 层不引入 Host/`PUBLIC_BASE_URL` 概念，`feed_cache_get`/`feed_cache_get_entry`/`feed_cache_put` 签名不变、只接受该键），**作用域由 server 侧 `_feed_cache_key` 组合**；`_feed_fetch_locks` 亦须按**同一 `cache_key`** 建锁（否则两个 Host 共享锁、双检串用对方的表示）。**放大风险论证与接受**：① 上游 JSON 取数另有 **URL 级缓存**（handler 内 `fetch_json(..., ttl=cache_policy('news_url')['ttl'])`）⇒ 键增多**不放大上游请求**（同一上游 URL 仍只取一次）；② 条目数受 `cache_policy('feed')['cache_max']=100` + **LRU** 约束；③ **非法 Host 经 `_valid_host_header` 一律塌缩为 `localhost:PORT` 单键**（不能靠畸形 Host 枚举）。故按"键覆盖维度 + 有界 + 不可枚举"三点接受该设计。

> ★ **v1.10 / F8 论证边界补正（`BR-SRV-45` 补正 / `BR-SRV-50`，P8 F8）**：上列 ①–③ **只证明"不放大上游请求"，未覆盖锁表内存与本地生成**——F2 后键空间 = `path × 请求派生 base_url`，而 `_valid_host_header`（`server.py:591`）**只校验格式、不校验归属** ⇒ **任意合法主机名可造新键**：**(a)** `_feed_fetch_locks` 若只增不减 ⇒ **永久内存增长**（每键约 300–450 B，外部可无界触发直至 OOM）；**(b)** 键数超 `feed_cache` 上限 100 后每个新 Host 必 miss ⇒ 一次 `generate_rss` + sha256、LRU 持续抖动（**只放大本地 CPU，上游仍被 URL 级缓存兜住**）。**"上游请求不放大" ≠ "本地资源不放大"**——后者的界由 **F8 提供**：`_get_or_fetch_feed` 的加锁改经 `cache.feed_fetch_acquire(cache_key)` / `feed_fetch_release(cache_key)`（**`try … finally`**，异常路径也释放），引用计数归零即**两表同删**，锁表规模**收敛于"并发在飞的键数"**、与"累计见过的键数"无关（`PUBLIC_BASE_URL` 已设时恒为 **5 条 path**）。**键语义（`BR-SRV-45`）与锁生命周期（`BR-SRV-50`）必须成对落地**——只落 F2 会把风险从"上游请求放大"移交给"锁表内存无界"。不变式与 TS-4 双抓回归网见 §2.2 R-7；资源行见 §4.3。

**`_feed_ttl_minutes()` ↔ `max-age` 一致性不变式（★ v1.9 / F4 新增，可断言）**：`_feed_ttl_minutes() == ceil(feed 路径的 max-age / 60)`——`<ttl>`（分钟）、feed 缓存 TTL、`Cache-Control: max-age` 三者**同读 `cache_policy('feed')` 单一 authority**（盘 30→1 / 非盘 180→3）；`_CACHE_AGE_DOMAINS` 的 5 个 feed path 由 `news_url` 改映射 `feed` 后，"两个 authority 可静默漂移"的缺口闭合。
...
**影响**：`server.py` 新增 `_feed_etag`/`_feed_fingerprint`（与 ETag 同源的规范化摘要）/`_feed_cache_key`（**v1.9 / F2**）/`_if_none_match_matches`/`_if_modified_since_not_modified`/`_not_modified`/`_feed_ttl_minutes`/`_send_not_modified`，`_serve_feed` 增条件分支、`_send_text` 增两可选形参；`cache.py` 新增 `feed_cache_get_entry`（既有 `_feed_cache_lock` 下的浅拷贝访问器，**v1.9 起返回六字段** `xml/time/last_modified/fingerprint/last_access/expires_at`）与 `feed_cache_put(path, xml, ttl, fingerprint=None) -> dict`（**v1.9 / F1+F5**：指纹继承 + **返回写入条目浅拷贝**，**不改** `feed_cache_get` 签名）；`utils.py` 的 `generate_rss` 增 `ttl=None` 可选形参。**无新锁**（校验器为纯函数 / 只读 `self.headers`）；**feed 条目由四字段扩为六字段属形态变更**（手改 feed 条目的旧代码会 `KeyError`，见 §7.1 N4）。**成本**：每次 RSS 请求一次 ≈40KB sha256 + 3 次正则 ≈ **0.1–0.2ms**（远小于同响应 gzip 与网络收益）。**AC**：**AC-A13**（PRD v0.7 由 prd-writer 并行落号；条件请求总语义 / F1·F3·F4·F5·F6）；**F2 键作用域另承接 AC-A14**（PRD v0.8 落号；见 §8）。
...
| **metrics.py（新增）** | 计数/仪表 + `snapshot()`；被 server/stream/cache/stock_api/market_api/cdp_engine 引用。**v1.4 实测口径**：`_KNOWN` 注册表（**v1.9：19 → 20 名**，新增 `http_304_total`）+ `_DEFAULTS` 零值表（import 期 `assert` 互锁）、`_LABELED_GAUGES` 形状守卫、`snapshot()` 零值恒定 + 锁内浅拷贝/锁外深拷贝、`reset()`（测试）。**规模已从设计期的 ~60 行增至 185 行**（形状守卫/深克隆/告警去重），仍为零依赖叶子模块。★ **v1.9 / F3**：`http_304_total` 由 `server._send_not_modified` 单点写入（`metrics.py` 本身零代码变更，仅注册表 + 零值表契约内容新增 1 名） | RC-4（观测）、AC-S10 | S10 | **L** |
...
**同层依赖登记（v1.4 补，REV-DES-10）**：`stock_api → cdp_engine`（经 `from .cdp_engine import page_data`）是**同层**依赖（两者都在 Layer 1），**无环**：`cdp_engine` 的 `forbiddenImports` 已含 `stock_api`，反向不可能成立。该边也必须写进 `tech-stack.json`，否则 `check-arch-compliance.sh` 的层级图与 §3 不一致。**（★ v1.12 核实：`tech-stack.json` 的 `layerIsolation` 已含该边——`china_finance_rss/stock_api.py` 条目 `forbiddenImports` 仅列 `server`/`stream` 且 `reason` 显式登记"允许同层单向引用 `cdp_engine.page_data`（无环）"，`china_finance_rss/cdp_engine.py` 条目反向 `forbiddenImports` 含 `stock_api`；本轮复核确认，无规则变更。）**依赖全图（同层边以 ⇢ 标注）：
```
...
| **（v1.8 例外 7）RSS 条件请求：200 新增 `ETag`/`Last-Modified`、条件命中新增 `304`、feed XML 新增 `<ttl>`** | 🟠 STABLE「只增」：**无任何条件头时 200 body/状态码逐字不变**（`ETag`/`Last-Modified` 为**新增响应头**；`304` 仅在客户端带匹配校验器时出现，属**显式 opt-in**）；既有 `Vary`/`private` 反投毒、`Cache-Control` 与 304 同值、业务降级恒 200 全部保持。**须记变更日志 + 同步 `API.md`**（含「`<ttl>` 非时效保证、最小 1 分钟、需 <60s 用 SSE」语句，见 §7.1 N4）。**★ v1.9 / F1–F6**：头项**零新增**（仅收紧 `Last-Modified` 语义为"表示最后一次变更的时刻"、内部键含 `base_url`、`max-age` authority 改 `feed` 数值不变、新增 `http_304_total` 观测名）——见 §2.7 |
...
| 既有字段类型与含义：尤其 `null = 无数据` | 只增不改（**例外**：`feeds[].status` 取值变更——**已由编排层批准并同步 `API.md:499-511`**，见 §7.1 Q2） |
...
| **RSS 条件请求命中（304）** | — | `_feed_etag` ≈40KB sha256 + 3 次 regex ≈ **0.1–0.2ms** + 少量头写出；**零 body、零 gzip、零上游回源**（回源仍由 feed TTL 决定） | **v1.8**：省的是客户端**每次轮询 ~37–45KB body**（实测 `/cls/telegraph` `Content-Length: 45003`、`/jin10/flash` 12901），**不是**上游请求（§2.7）。**★ v1.9 / F1**：`Last-Modified` 改表"最后一次变更"后，**IMS-only 客户端跨 TTL 亦得 304**（原"跨 TTL 必得 200"的取舍已消除）⇒ 带宽收益对两类条件头客户端一致 |
...
⇒ 池上限 = pool_max = **按域声明**：`'dedup'` ⇒ MAX_DEDUP_CODES (2000)；`'fixed:N'` ⇒ N；`'env:<NAME>'` ⇒ env 注册常量（调用期白名单解析，v1.15 CR-02）。
...
**内存分项估算（进程内，2C2G；★ v1.14 PRF-MEM-01 重算）**：URL 20MB + feed 30MB + **端点数据缓存**（`cache_max` 按域声明，**改写 v1.7 的"3 域×2000×~8KB≈48MB"旧口径**，按实测单条原始 JSON 体积报价）：**quote 1000×~9.4KB ≈ 9.4MB + fundflow 1000×~0.6KB ≈ 0.6MB + timeline 500×~96KB ≈ 48MB（**内存大户，占主导**）+ depth ≤500×~0.36KB ≈ 0.2MB** ≈ **58MB** + f10/announcement ≈11MB + sector 0.2MB + **`_last_known` ≤2000 码 × 3 字段（引用同一批 dict/数值对象，≈1MB 保守计，v1.4）** + SSE 队列 ≤128MB + 解释器/栈 ≈30MB ≈ **278MB**（**上界一：SSE 队列按满配 128MB 计**；空闲态实测远低于此）。**★ v1.16 口径限定：此 ≈278MB 为「已知缓存项」上界，非进程总内存上界**——实测 python RSS `1.012GiB` 与之相差 **≈750MB**（缺口候选：共享 URL 缓存解析后对象 + 线程池 glibc arena 碎片 + CDP/运行时；**登记 AR-18**）。
> ⚠️ **口径声明（v1.14 PRF-MEM-01；v1.15 CR-05 更正 prefetch 间隔口径；★ v1.16 实测更正/回填）**：上列按**单条原始 JSON 体积**折算，仅作**量纲参考**；**展开为 Python 对象后实际占用数倍于原始体积**，故**权威口径以压测实测为准**。**★ v1.16 实测（权威，run 20260920-110805）**：变更前 python 进程 RSS **`1.095GiB`**（容器 `1.449GiB/1.5GiB = 96.57%`；缓存条目 fundflow=2000/quote=2000/depth=500/timeline=1350 均达 `cache_max`；`cache_hit_ratio=0.1585`；`upstream_timeout=123`）→ 池/缓存按域收缩后 python 进程 RSS **`1.012GiB`**、容器 **`1.41GiB = 93.98%`**（3 域配置经 healthz 精确生效：quote/fundflow=1000/1000、timeline=500/500），**净收益仅 ≈`83MiB`** ⇒ **未达成内存目标**（原 v1.14/v1.15 的"预期 ≈0.65GiB"**被实测证伪**）。**归因更正（取代 v1.14 的"稳态内存 ≈ `Σ(cache_max × 单条展开体积)`"单一归因）**：① 终端缓存收缩是**小头**——按单条原始体积反推删除量 ≈**92MB**（quote 1000×~9.4KB + fundflow 1000×~0.6KB + timeline 850×~96KB），与 −83MiB 净收益吻合；② **真正大头 = 共享 URL 缓存**——`cache.MAX_CACHE_SIZE=2000`（`cache.py:35`）实测 `cache_entries.url=2000` **顶满**，条目为**解析后的 Python 对象**（远超上表按原始 ~10KB/条的"URL 20MB"报价）；③ 其次为**线程池 glibc arena 碎片**（prefetch 按 **`pool_refresh`（= `ttl × refresh_factor`；盘中 quote 4s、fundflow/timeline 8s，非盘 120s；`depth` 无 prefetch 循环——★ v1.15 CR-05 更正，原"每 120s"为误述）** 轮询 pool 续期，20 批 worker 高频分配/释放）。**★ 新增架构事实（CR-01 预警被实测证实）：「缓存上界 ≠ 进程 RSS」**——本表推导的**已知缓存项**上界 ≈**278MB** 与实测 python RSS **`1.012GiB`** 相差 **≈750MB**，即**账目只覆盖已知缓存项、未覆盖分配器（glibc arena）/CDP/运行时开销** ⇒ **登记为待解架构项 AR-18**（后续方向：**共享 URL 缓存调优**）。**旁证（待复测归因，非结论）**：`cache_hit_ratio` 0.1585 → **0.1796**；tier1000 timeline `ok_rate` 100% → **89.6%**、`upstream_timeout` 123 → **317**（**标注待复测归因，不作为结论**）。
...
**影响**：`_feed_etag`/`_feed_fingerprint`（v1.9 补）/`_not_modified`/`_if_none_match_matches`/`_if_modified_since_not_modified`/`_send_not_modified`；~~`Last-Modified` 取缓存写入时刻（IMS-only 客户端跨 TTL 仍 200，已登记）~~ **（★ v1.9 / F1：已由下方追加块修订为"表示最后一次变更的时刻"，IMS-only 跨 TTL 亦 304）**；304 无 body / 无 `Content-Encoding` / 无 `Content-Length`；`<ttl>` advisory 最小 1 分钟（BR-SRV-43）；**仍为拉模型、HTTP/1.0、未启用 keep-alive**。**可逆性判据**：撤回只需删条件分支与两个可选形参；但 ETag 已发布给下游、`<ttl>` 可能诱导聚合器调整轮询，故定为**低可逆（对外契约）**。**AC**：**AC-A13**。
**何时改变选择**：若上游提供稳定版本号 / 条目级时间戳（可作**强**校验器），可改为版本比进而放弃规范化投影；若引入 CDN，条件请求语义须交 CDN 统一实现。

> **★ v1.9 修订（P8 F1/F2 追加，不重写历史）**
> **（a）F1 选型——`Last-Modified` 时间源 = 表示最后一次变更的时刻**：v1.8 原取 feed 条目**写入时刻**（`entry['time']`），导致 IMS-only 客户端**跨 TTL 必得 200**（写入时刻每次回源前进）。修订为：feed 条目承载 `fingerprint`（**与 ETag 同源的规范化摘要**）与 `last_modified`；`feed_cache_put` 与**同一键上一条目**（★ **即使已过期也参与比较**）比指纹——相同 ⇒ **继承** → 不同 ⇒ 本次写入时刻；不变式 **`ETag` 变 ⟺ `Last-Modified` 前进**。**否决备选 `Last-Modified = max(items.pubDate)`**（免状态）：上游"改描述不改 `pubDate`"会发出**错误 304（陈旧数据）**，对交易数据服务**错误 304 远比多发 200 严重**，故否决。
> **（b）F2 裁定——`Vary` 声明了什么，源站自身缓存键就必须覆盖什么**：feed body 内嵌**请求 Host 派生**的 `<atom:link rel=self>`。v1.8 键仅 `path`，则一条**伪造 Host** 的请求就能在一个 TTL 内**改写所有读者的订阅链接**，与 `Vary: Host, X-Forwarded-Host, X-Forwarded-Proto` 声明不符。修订：`PUBLIC_BASE_URL` 未设时键 = `path + '\x00' + base_url`（已设时 = `path`，逐字不变）；键对 cache 层**不透明**，作用域由 server 侧 `_feed_cache_key` 组合；`_feed_fetch_locks` 按同一 `cache_key` 建锁。
> **（c）F2 放大风险论证与接受**：① 上游 JSON 取数另有 **URL 级缓存** ⇒ 键增多**不放大上游请求**；② 条目数受 `cache_policy('feed')['cache_max']=100` + **LRU** 约束；③ **非法 Host 经 `_valid_host_header` 一律塌缩为 `localhost:PORT` 单键**（不可枚举）。三点成立 ⇒ 接受该设计。
> **可逆性不变**：仍为**低可逆（对外契约）**；本次仅**收紧语义与内部键作用域**，不新增对外头项。**AC**：AC-A13（F1·F3·F4·F5·F6 语义收紧；**F2 键作用域承接 AC-A14**——见 §8；回归报警另见 `http_304_total` / §2.6）。

**ADR 汇总**：**23 条**（低可逆 **11**：001-005 + **013** + **016** + **017** + **018** + **020** + **023**；中可逆 **12**：006-012 + **014** + **015** + **019** + **021** + **022**）；高可逆决策（如具体常数值、日志文案、"tick 网格 vs 0.25×tick 下限"这类一个函数即可回滚的节拍规则）不记 ADR（tick 节拍落点见 §2.6）。**v1.9 未新增 ADR 编号**——F1/F2 均为 ADR-023 的修订（追加）。
...
**tech-stack.json 已更新于 `doc/arch/tech-stack.json`（**v1.16**；v1.9–v1.11 的 `backend.cache` / `namingRules` / `architectureRules.metrics` 内容变更见 §9.9–§9.11；**v1.14/v1.15/v1.16 仅版本号联动**——PRF-MEM-01 的 `pool_max`/`cache_max` 数值与来源权威在 `config.py` 与 SAD §2.1/§4.3，`tech-stack.json` 无该口径可同步，见 §9.14/§9.15/§9.16）**：含 `architectureRules`——`importRestrictions.denylist` 固化"零第三方库"红线；`allowlist` 补入 `websocket`（仅函数内延迟导入，修 P2-7）；`layerIsolation`/`fileStructure` 与 §2.6/§3 的模块划分一致，供 `code-developer` 自验与 `check-arch-compliance.sh` 校验。**帧计费（`_Frame`/refs/`stream_queue_bytes`）归 stream 层，cache.py 不得承载**——由现有 `layerIsolation`（cache.py 禁 import stream/server/stock_api/market_api）覆盖；**feed 缓存机制归 cache 层、`socket` 已在 allowlist**（D-3）；**v1.7：HTTP 连接池/DNS 缓存同属 cache 层**（仍是 stdlib `http.client`/`socket`，不新增依赖）。
...
| Q2 | 三条 CDP 标注漂移（`/stock/data` `/stock/basic_info` `/stock/f10`） | 分类 **B / B / A′** 与 PRD §7.2 一致，确认；**同步权归编排层 → 已由编排层行使**；SAD **删除**"不改既有 `feeds[]` 字段与含义"；`feeds[].status` 属**改既有字段取值**。**✅ 已同步（`API.md:499-511`）**——明文列取值 `configured`/`ok`/`error`/`requires_chrome_cdp`，并写明 `/stock/data`·`/stock/basic_info` = `configured`（纯 REST）、`/stock/f10` = `requires_chrome_cdp`（即详设 `server.md` §2.9.1 三处目标值） | §2.6 / §3 / 下方"契约同步项" |
...
| **N4（新，v1.8 P7b；v1.9 核对更新）** | **RSS 条件请求（`ETag`/`Last-Modified`/`304` + `<ttl>`）为 🟠 STABLE「只增」对外变更**：5 个 feed 的 200 新增 `ETag`/`Last-Modified` 头、条件命中返回 `304`、feed XML 新增 `<ttl>`。**★ v1.9 / F1–F6 核对更新**：(a) **HTTP/1.0 下 304 无 `Content-Length`、由 EOF 收尾**（RFC 9110 §8.6 **禁发 `Content-Length: 0`**；本轮**不升 `protocol_version`/keep-alive**）——中间件对 EOF 收尾的处理属**理论风险**，`http.client` 已实测通过（F6 / §2.7）；(b) **`feed_cache_put` 返回值契约**：新增第 4 位可选 `fingerprint` 且**返回写入条目浅拷贝（非 `None`）**，miss 路径**删除 put 后第二次查询**（F5）；(c) **feed 条目四→六字段的「形态变更」**（`xml/time/last_modified/fingerprint/last_access/expires_at`）——**任何手改 feed 条目的旧代码会 `KeyError`**，属内部形态变更、须记入变更日志并同步相关脚本/测试桩；(d) **★ v1.11 / P2-1 · `last_modified` 缺字段 fail-safe**：cache 层以 `entry.get('last_modified')` / `prev.get('last_modified')` 读取 ⇒ **legacy / 非六字段条目缺该字段 ⇒ `None` ⇒ 不发 `Last-Modified`、禁用 IMS**（`If-None-Match` 仍按 ETag 评估），**绝不抛 `KeyError` 经 `_guard` 变降级体**（详设 `server.md` v1.11 / `cache.md` v1.10）。 | 归入「只增」（无条件下 200 body/状态码逐字不变；`304` 为客户端**显式 opt-in**）；**须记入变更日志 + 同步 `API.md`**：必须写入「**`<ttl>` 是聚合器缓存提示、非时效保证；最小粒度 1 分钟；推荐 `ETag` 条件请求优先、间隔 ≥30s；需 <60s 用 SSE（4s）**」；**原"仅带 `If-Modified-Since` 的客户端跨 TTL 边界会完整重取"随 F1 取消**（`Last-Modified` 现表"最后一次变更"⇒ 跨 TTL 指纹未变仍 304，两类条件头等价） | §2.7 / §3.1 例外 7 / ADR-023 / §7.3#12 |

**CDP 标注契约同步项（Q2）——✅ 已闭环（v1.12）：health 负载 + 首页 CDP 列 + `API.md` 三处取值均已同步（`API.md:499-511` 明文列 `configured`/`configured`/`requires_chrome_cdp`）**
...
| `/stock/basic_info` | API.md 与 health 双错 | `configured`（**B**） | ✅ **全链路已同步**：health 负载 + 首页（`configured` / `needs_cdp=False`）+ **`API.md:499-511` 已列 `configured`（明文"纯 REST"）**；`API.md:244` 的 `/stock/basic_info` 已无"⚡ 需要 Chrome CDP"标注（`API.md:226` 的 `/stock/f10` 仍标 ⚡，正确） |
| `/stock/f10` | health `configured` + 首页 `needs_cdp=False` | CDP（**A′**） | ✅ 已落地（health = `requires_chrome_cdp`；首页 `needs_cdp=True`）；API.md 原本即"是" ✓ |

> **结论（v1.12 收口）**：Q2 **全链路已闭环**——代码侧（`server.py`）+ `API.md`（`:499-511` 明文取值；`:244` `/stock/basic_info` 已去 ⚡；`:226` `/stock/f10` 保留 ⚡）均已同步。剩余仅**变更日志登记**（编排层动作，不阻塞契约）。
...
| **AR-18（v1.16 新）** | **「缓存上界 ≠ 进程 RSS」——内存账目缺口 ≈750MB**：SAD §4.3 推导的**已知缓存项**上界 ≈**278MB**，而 PRF-MEM-01 实测 python 进程 RSS **`1.012GiB`**（收缩后），二者相差 **≈750MB**；§4.3 账目**只覆盖已知缓存项、未覆盖分配器（glibc arena）/CDP/运行时开销**（**CR-01 早已预警，现被实测证实**）。由此 **「调小 `cache_max` ⇒ 进程内存线性下降」的推断不成立**（v1.14→v1.16 实测：终端缓存收缩按原始体积反推 ≈92MB，仅兑现 **≈83MiB** 净收益） | **高** | ① **待解架构项**，后续方向 = **共享 URL 缓存调优**（`cache.MAX_CACHE_SIZE=2000` 顶满、条目为解析后 Python 对象，实测 `cache_entries.url=2000`；评估**降上限 / 缩短 TTL / 按域分区 / 更紧凑的存储表示**）；② **线程池 glibc arena 碎片**（`MALLOC_ARENA_MAX` / `malloc_trim` / worker 数）纳入专项；③ **内存账目口径更正**：§4.3 明确标注为「**已知缓存项上界**」，**禁止**当作进程总内存上界（§2.2 R-4 / §4.3 / §8 S9 同步）；④ 需按**进程 cgroup 实测**（RSS/PSS 分列）建立总账，**不得**再用 `Σ(cache_max × 单条体积)` 外推进程内存；⑤ AC-S9 24h 采样 + 重建部署后复测 |
...
| **2.7 RSS 条件请求：规范化体弱校验器 `_feed_etag`（剔 `lastBuildDate`/`ttl`/**`pubDate`**）+ `Last-Modified` 取 feed 条目 `last_modified`（★ v1.9 / F1：**表示最后一次变更的时刻**，指纹跨 TTL 继承）+ 304 组成 / 条件头优先级（v1.8，ADR-023；评审 D-1 漂移回填）** | 本专项 R1（**非根因编号**；见 §2.7 命名澄清）/ 带宽 | **AC-A13** |
| **2.7 `<ttl>` advisory（最小 1 分钟；盘 1 / 非盘 3，剔除出 ETag）+ 仍为拉模型 / HTTP/1.0 无 keep-alive（v1.8）** | — | **AC-A13** |
| **2.7 ★ v1.9 / F1：`ETag` 变 ⟺ `Last-Modified` 前进（不变式）；IMS-only 跨 TTL 亦 304（旧取舍消除）** | 本专项 R1 / 带宽 | **AC-A13** |
| **2.7 ★ v1.9 / F2：feed 缓存键覆盖表示维度（`path [+ '\x00' + base_url]`，与 `Vary` 同语义；cache 层视键不透明）** | P1-2（跨 Host 订阅链接劫持） | **AC-A14** |
| **2.7 ★ v1.9 / F3：`http_304_total`（`_send_not_modified` 单点；注册表 19 → 20）** | — | **AC-A13** / S10 |
| **2.7 ★ v1.9 / F4：RSS `max-age` authority = `feed` 域（数值不变）+ `_feed_ttl_minutes() == ceil(max-age/60)` 不变式** | A3/A8 | **AC-A13** |
| **2.7 ★ v1.9 / F5：`feed_cache_put -> dict`（写条目浅拷贝）+ miss 无二次查询；条目四→六字段** | A8 / S1 | **AC-A13** |
| **2.7 ★ v1.9 / F6：HTTP/1.0 304 EOF 收尾、不发 `Content-Length: 0`（RFC 9110 §8.6）** | E1 / 连接模型 | **AC-A13** |
| **2.2 ★ v1.10 / F8：feed 取数锁表引用计数回收（`feed_fetch_acquire/release` + `_feed_fetch_refs`；计数先于返回锁、归零两表同删；不变式"计数 ≥1 ⇒ 不可 pop"；有界 = 并发在飞键数；TS-4 双抓回归网）——`BR-SRV-50` / `BR-CACHE-35` ↔ 测试 `SRV-T71`/`T72`/`T-CACHE-35`** | P1-1（P8 盲审；锁表只增不减） | E6/E7/**S12**（AC-S12 per-key 资源收敛，**主承接**；并列 **S9** 24h 资源总账）；并发不双抓保 AC-S1 |
...
| S4 | ✅ | §2.3 D-4 三分（B/B/A′）+ **D-4 补充 2（v1.4：`page_data`/窗口终态/`_last_data` 老化/导航锁限时）**；**health 负载 + 首页 CDP 列 + `API.md:499-511` 三处取值均已同步**（v1.12 由 ⚠️ 改 ✅：Q2 唯一残留项 `API.md` 契约同步闭环，见 §7.1 Q2） |
| S5 | ✅ | §2.2 503 + 撤除后恢复 |
...
| **AC-A14** | ✅ | **v1.9 承接（PRD v0.8 由 prd-writer 并行落号；v1.11 落号对齐：AC 归属由 AC-A13 更正为 AC-A14）** §2.7「feed 缓存键的表示作用域」+ §5 ADR-023 追加（b）（c）：**键必须覆盖表示的全部可变维度**——`PUBLIC_BASE_URL` 已设 ⇒ `cache_key = path`（与 Host 无关）；未设 ⇒ `path + '\x00' + base_url`（不同 Host 互不串号，防一条伪造 Host 在一个 TTL 内改写全体读者订阅链接）；键对 `cache.py` **不透明**、作用域由 server `_feed_cache_key` 组合，`_feed_fetch_locks` 按**同一 `cache_key`** 建锁；**放大风险论证与接受**（① 上游 JSON 取数另有 URL 级缓存 ⇒ 不放大上游请求；② `feed_cache` 受 `cache_max=100` + LRU 约束；③ 非法 Host 经 `_valid_host_header` 塌缩为 `localhost:PORT` 单键）；**★ v1.11 / F8 论证边界补正**：上述三点只证"上游请求不放大"，**"本地资源不放大"由 AC-S12 / §2.2 R-7 提供界**；详设 `server.md` **BR-SRV-45** + `cache.md` **BR-CACHE-32**（键不透明）/ BR-CACHE-33；测试 **`SRV-T65`**。 |
| **AC-S12** | ✅ | **v1.10 承接（PRD v0.9 由 prd-writer 并行落号；v1.11 补独立追溯行）** §2.2 **R-7「进程内 per-key 资源生命周期」** + §2.7 F8 有界性结论 + §4.3 锁表上界行：feed 取数锁表（`_feed_fetch_locks` + `_feed_fetch_refs`）与键空间**同界收敛**——`cache.feed_fetch_acquire/release` 引用计数在 `acquire` 时 +1 且**先于返回锁**、`release` 在 `finally` 递减、**归零两表同删**；不变式"计数 ≥1 ⇒ 该键不可 pop"（归零前 `pop` ⇒ 同键双抓 = 历史事故 **TS-4**）；**锁表规模 = 并发在飞的键数**（`PUBLIC_BASE_URL` 已设时恒 **5 条 path**），**不随累计见过的 Host 数增长**；详设 `server.md` **BR-SRV-50** / `cache.md` **BR-CACHE-35**；测试 **`SRV-T71`·`T72`·`T-CACHE-35`**。 |

**覆盖声明**：PRD 全部 AC 均有设计落点（v0.6 为 33 条；**v1.8 追加 AC-A13 后 34 条**；**v1.9 承接 AC-A14 → 35 条、v1.10 承接 AC-S12 → 现行 36 条**（高效 10 / 准确 14 / 稳定 12）；E7/E9/S6/S11 的原缺口已补齐，见上表）；其中 **4 条标注 ⚠️**：E4（命中率降为观测项，Q1）、A3（条件 C1 不成立时降级，P1-1）、A8（TTL 口径同步，Q3）、S3（模式 B 分档 P95 待 P6c 校准，Q6）（**v1.12：S4 由 ⚠️ 改 ✅**——CDP 契约同步闭环，`API.md:499-511` 已列三处取值；⚠️ 数 5 → 4）。**v1.5：A5 由 ⚠️ 改 ✅**（N1 裁决关闭——业务降级恒 200，与 AC-A5 逐格一致；§7.1 N1）。**测试方法与阈值**由 task-decomposer→tester 阶段落实，本 SAD 保证"机制存在且可断言"。**v1.4 的"A5 裁决未落地前不得进入基线"前置约束已随 N1 关闭解除**；AC-A5 回归**须含"业务降级不得返回 503"断言**（§9.5 遗留 1）。**v1.7**：设计点矩阵新增 5 行（订阅驱动节拍 / `refresh_epoch` / 上游传输层 / 线路拼写+空壳防御 / `depth` 并入 / 发送层去重），**（当时）30 条 AC** 的**覆盖状态不变**（⚠️ 仍 5 条：E4/A3/A8/S3/S4），A3/E1-E3/A1 的**数值与口径**已按 `coverage=213`、`BATCH_MAX_WORKERS=20`、`_PER_FETCH_EST=0.3` 与传输层实测重标（见 §9.7）。 **v1.8**：设计点矩阵新增 2 行（RSS 条件请求 / `<ttl>`+拉模型），**新增 AC-A13**（PRD v0.7 由 prd-writer 并行落号；AC 总数 33 → **34**），⚠️ 数量不变（仍 5 条：E4/A3/A8/S3/S4）。 **v1.9**：设计点矩阵新增 **6 行**（F1 `Last-Modified` 语义 / F2 键作用域 / F3 `http_304_total` / F4 max-age authority / F5 put 返回值 / F6 304 EOF），**承接 AC-A14**（F2 键作用域；PRD v0.8 由 prd-writer 并行落号；**F2 的 AC 归属由 AC-A13 修正为 AC-A14**，F1·F3·F4·F5·F6 仍承接 AC-A13，A13 由 ✅ 保持），**AC 总数 34 → 35**，⚠️ 数量不变（仍 5 条：E4/A3/A8/S3/S4）。 **v1.10**：设计点矩阵新增 **1 行**（F8 feed 取数锁表引用计数回收，`BR-SRV-50`/`BR-CACHE-35` ↔ `SRV-T71`/`T72`/`T-CACHE-35`），**承接 AC-S12**（PRD v0.9 由 prd-writer 并行落号；**F8 主承接编号 = AC-S12**，`AC-S9`「24h 资源总账」保留为并列），**AC 总数 35 → 36**，⚠️ 数量不变（仍 5 条：E4/A3/A8/S3/S4）。**（本版 v1.11 对上述两处归属做落号对齐；**v1.12 把 S4 由 ⚠️ 改 ✅（Q2 `API.md:499-511` 契约同步闭环）⇒ 现行 ⚠️ 4 条（E4/A3/A8/S3）**；全文历史 §9.x 记录块中的"当时 34 条"为历史快照，现行计数以本节为准。）**
...
| 2 | **`Last-Modified` 时间源** | `_get_or_fetch_feed` 返回 **2-tuple `(xml, last_modified)`**（`:1307-1320`）；`last_modified = feed_cache_get_entry(path)['time']`（`cache.py:906-924`，浅拷贝 + 既有 `_feed_cache_lock`）；降级路径 `(error_xml, None)`（`server.py:1325-1328`）。**★ v1.9 / F1 起改取 `entry['last_modified']`（表示最后一次变更的时刻，见 §9.9）；本行 `['time']` 为 v1.8 历史事实** | §2.7 / §3 cache·server 行 |
...
| 2 | **`API.md` / README / 变更日志**补 `ETag`/`Last-Modified`/`304`/`<ttl>` + 「`<ttl>` 非时效保证、最小 1 分钟、需 <60s 用 SSE」+ ~~IMS-only 客户端跨 TTL 会完整重取~~ **（★ v1.9 / F1 作废此句：`Last-Modified` 现表"最后一次变更"，IMS-only 跨 TTL 亦 304；见 §9.9）**（评审 D-6 / code-reviewer 待办 ⑤⑥） | 契约同步（编排层，N4） |
...
### 9.9 v1.9（P8 对抗性盲审 F1–F7 架构回填）

> **性质**：RSS 条件请求（v1.8 承接）经 **P8 对抗性盲审**暴露 **2×P1 + 8×P2**，编排层裁定修复 **F1–F7**；详设先行升级（`server.md` **v1.9**（BR-SRV-38 改写 + 新增 BR-SRV-45..49）/ `cache.md` **v1.8**（BR-CACHE-32 改写 + BR-CACHE-33/34）/ `metrics.md` **v1.4**（BR-MET-14））、**代码已实现**。本轮把**架构级结论**回填 SAD，范围严格限定 `doc/arch/SAD.md` + `doc/arch/tech-stack.json`；**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`**。**一切以详设 v1.9/v1.8/v1.4 为准**（不臆造未实现设计）。**只增不删编号**：无新增 AR/ADR/N，修订就地标注（ADR-023/AR-16 追加，不重写历史）。

**一、回填条目（逐项：裁定 → SAD 落点）**

| # | 裁定 | 内容 | SAD 落点 |
|---|------|------|---------|
| **F1** | **P1-1 根治** | `Last-Modified` = **表示最后一次变更的时刻**：feed 条目承载 `fingerprint`（与 ETag 同源）+ `last_modified`；`feed_cache_put` 与**同一键上一条目**（★ 即使已过期）比指纹 ⇒ 相同继承、不同取本次写入时刻；不变式 **`ETag` 变 ⟺ `Last-Modified` 前进**；否决备选 `max(items.pubDate)`（改描述不改 pubDate ⇒ **错误 304**，比多发 200 严重） | §2.7（`Last-Modified` 改写 + 旧取舍取消）/**AR-16 修订**/**ADR-023 追加**/§8 |
| **F2** | **P1-2 根治** | feed 缓存键覆盖表示全部变化维度：`PUBLIC_BASE_URL` 已设 ⇒ `path`；未设 ⇒ `path + '\x00' + base_url`；键对 cache **不透明**、作用域归 server `_feed_cache_key`；`_feed_fetch_locks` 按同一键；放大风险论证（上游 URL 缓存 ⇒ 不放大；`cache_max=100`+LRU；非法 Host 经 `_valid_host_header` 塌缩 `localhost:PORT` 单键） | §2.7（键作用域）/**ADR-023 追加**/§3 server 行/§8 |
| **F3** | **P2-6** | 新增 `http_304_total`：`metrics._KNOWN` + `_DEFAULTS` **同步注册**（19 → **20**）；`server._send_not_modified` **单点**计数；只需计数不需分母（单调，停止增长即"永远 200"回归报警） | **§2.6 计分板**/§3 metrics·server 行/§8 |
| **F4** | **P2-4** | feed `max-age` authority 由 `news_url` 改 **`feed` 域**；**数值不变、契约不变**（盘 30 / 非盘 180）；新增不变式 `_feed_ttl_minutes() == ceil(max-age/60)` | **§2.1 D4 推导强化**/§2.7/§3 server 行/§8 |
| **F5** | **P2-5** | `feed_cache_put(path, xml, ttl, fingerprint=None) -> dict` **返回写入条目浅拷贝（六字段，非 `None`）**；`_get_or_fetch_feed` 直接消费、**删除 put 后二次查询**（消除「200 带 ETag 却不带 `Last-Modified`」窗口） | §2.7 影响/§3 cache·server 行/**N4**/§8 |
| **F6** | **P2-7 仅文档化** | HTTP/1.0 下 304 **无 `Content-Length`**、由 **EOF** 收尾；RFC 9110 §8.6 明令 304 **不得**发 `Content-Length: 0`；**本轮不升 `protocol_version`/keep-alive**；中间件收尾为**理论风险** | §2.7 适用范围内补记/**N4**/§8 |
| **F7** | **测试** | 新增 `SRV-T63..T70` 8 条 + 改写 `T55`（测试 64 → **72**）；`T-CACHE-33/34` 新增 + `T-CACHE-32` 扩六字段 | 详设侧（`server.md`/`cache.md`）；SAD §8 追溯 |

**二、文件动作**

| # | 文件 | 动作 |
|---|------|------|
| 1 | `doc/arch/SAD.md` | **v1.8 → v1.9**：头部版本 + v1.9 修订依据；§2.1 D4 推导强化；§2.6 计分板（+`http_304_total`）；§2.7（F1–F6 全部）+ 新增内部契约块（键作用域 / `_feed_ttl_minutes()` 不变式）；§3 cache/server/metrics 行；§3.1 例外 7；§4.1；§5 ADR-023 追加；§7.1 **N4** 核对更新；§7.2 **AR-16** 修订；§8 追溯矩阵 + AC-A13；§9 头部 + 本节。**未增删编号。** |
| 2 | `doc/arch/tech-stack.json` | **`version: 1.8 → 1.9`**：`backend.cache` 条目（`feed_cache_get_entry` 六字段 + `last_modified` 权威 + `feed_cache_put` 指纹继承/返回写入条目）；`namingRules.rss`（F1 语义 / F2 键 / F3 计数 / F4 authority / F6 EOF）；`namingRules.metrics`（注册表 20 名，+`http_304_total`）。`allowlist`/`layerIsolation`/`fileStructure`/`importRestrictions` 不变。 |

**三、与详设的一致性核对（无跨文档冲突）**

1. `server.md` BR-SRV-38（改写）、BR-SRV-45..49 / `cache.md` BR-CACHE-32（改写）、33、34 / `metrics.md` BR-MET-14 与 SAD 本版逐条一致；`metrics.md` v1.4 已把"注册表不新增（SAD 契约冻结）"改写为"除本次裁定外不再新增"，**SAD 侧已同步为 19 → 20**。
2. 命名以详设为准：非法 Host 判定函数为 **`_valid_host_header`**（非 `_valid_host_field`）。
3. `cache.md` 声明 `path` 为**不透明缓存键**；SAD §2.7 明确"作用域由 server 侧 `_feed_cache_key` 组合"，与 `layerIsolation`（cache 不感知 Host/`PUBLIC_BASE_URL`）一致。

**四、本轮遗留（交编排层）**

| # | 事项 | 性质 |
|---|------|------|
| 1 | **`API.md` / README / 变更日志**：补 `Last-Modified` 语义修订（跨 TTL 指纹未变仍 304）、`http_304_total`、feed 条目四→六字段形态变更、HTTP/1.0 304 EOF；并**删除**"仅带 `If-Modified-Since` 跨 TTL 会完整重取"旧句 | 契约同步（编排层，N4） |
| 2 | 详设 `server.md` §10.1 打桩迁移（`counting_put` 增 `fingerprint=` 形参、`FeedDoubleCheckTests` 六字段桩等）与测试 `SRV-T63..T70` 落地 | 已在详设登记（本轮 SAD 不改） |

**本轮版本**：SAD v1.8 → **v1.9**；`doc/arch/tech-stack.json` 同步 `version: 1.9`（**内容有实质变更**：cache/namingRules 条目）。**约束保持**：Python 3 标准库零依赖、`layerIsolation`、文件结构（扁平包 + 唯一新增 `metrics.py`）均未放宽。**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`。**

### 9.10 v1.10（P8 F8 架构回填：feed 取数锁表有界收敛）

> **性质**：RSS 条件请求（v1.8 承接、v1.9 回填 F1–F7）经代码评审 `doc/review/rss-conditional-get_p8修复_代码评审_专家版.md`（`CR-RSS-20260918-003`）查出 **P1：F2 引入的 `_feed_fetch_locks` 按含 `base_url` 的键建锁且全仓无淘汰 ⇒ 合法 Host 可枚举、锁表无界增长 + 每请求本地重生成 XML**；编排层裁定修复 **F8**，详设先行升级（`server.md` **v1.10** / `cache.md` **v1.9** / `metrics.md` **v1.5**）、**代码已实现、511 用例全绿**。本轮把**架构级结论**回填 SAD，范围严格限定 `doc/arch/SAD.md` + `doc/arch/tech-stack.json`；**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`**。**只增不删编号**：新增 **R-7**（§2.2），未删任何 BR/AR/ADR/N。

**一、回填条目（逐项：裁定 → SAD 落点）**

| # | 裁定 | 内容 | SAD 落点 |
|---|------|------|---------|
| **F8** | **P1（资源）** | feed 取数锁表与键空间**同界收敛**：`_get_or_fetch_feed` 加锁改经 `cache.feed_fetch_acquire/release`（**`try … finally`**）；`acquire` **计数先于返回锁**、`release` 归零**两表同删**；不变式"计数 ≥1 ⇒ 不可 pop"；**为什么不能简单 `pop`**（归零前 pop ⇒ 同键双抓 = TS-4）；有界性 = **并发在飞的键数**（`PUBLIC_BASE_URL` 已设时恒 5 条 path） | **§2.2 R-7（新增）** / §2.7（F8 有界性 + `BR-SRV-45` 论证边界）/ §3 cache·server 行 / §4.3 / §8 |
| **BR-SRV-45 补正** | 论证边界 | 原"放大风险论证"只覆盖**上游请求**，未覆盖**锁表内存与本地生成**；`_valid_host_header` 只校验格式不校验归属 ⇒ 任意合法主机名可造新键：(a) 锁表**永久内存增长**（每键 ~300–450 B）；(b) 超 `feed_cache` 上限 100 后每新 Host 必 miss ⇒ `generate_rss`+sha256、LRU 抖动（**只放大本地 CPU，上游仍被 URL 缓存兜住**）。**"上游请求不放大" ≠ "本地资源不放大"**，后者由 F8 提供界 | §2.7 |

**二、文件动作**

| # | 文件 | 动作 |
|---|------|------|
| 1 | `doc/arch/SAD.md` | **v1.9 → v1.10**：头部版本 + v1.10 修订依据；§2.2 新增 **R-7**；§2.7 补 F8 有界性 + `BR-SRV-45` 论证边界；§3 cache/server 行；§4.3 资源行；§8 追溯矩阵（F8 行 + 覆盖声明）；§9 头部 + 本节。**只增不删编号。** |
| 2 | `doc/arch/tech-stack.json` | **`version: 1.9 → 1.10`**：`backend.cache` 补 `feed_fetch_acquire/release` + `_feed_fetch_refs`（引用计数归零两表同删；唯一入口）；`namingRules.concurrency` 新增「per-key 资源引用计数回收」约定。`allowlist`/`layerIsolation`/`fileStructure`/`importRestrictions` 不变。 |

**三、与详设的一致性核对（无跨文档冲突）**

1. `server.md` **BR-SRV-50**（新增）/ **BR-SRV-45**（补正）/ `cache.md` **BR-CACHE-35**（新增，含 `feed_fetch_acquire`/`feed_fetch_release`/`_feed_fetch_refs`）/ `metrics.md` **v1.5** 与 SAD 本版逐条一致；测试 `SRV-T71`/`SRV-T72`（`server.md`）、`T-CACHE-35`（`cache.md`）。
2. 命名以详设为准：锁表 `_feed_fetch_locks`、计数表 `_feed_fetch_refs`、原语 `feed_fetch_acquire`/`feed_fetch_release`、共享锁 `_feed_fetch_locks_lock`。
3. **§2.6 计分板核对（"19 vs 20"）**：本版**直接读文件确认** §2.6 计分板 `http_304_total` 行已记「注册表 **19 → 20** 名」（§3 metrics 行、§8 F3 行、§9.9 同）——**无需修正**；并核对实现 `metrics._KNOWN` / `_DEFAULTS` 恰为 **20 名**且含 `http_304_total`（`metrics.py:20-56`，`assert set(_DEFAULTS) == _KNOWN`），即上一轮 v1.9 回填**已真实落地**。全文未再发现"19 个度量名 / 注册表冻结不新增"的现行表述（`metrics.md` v1.4 已改为"除本次裁定外不再新增"，与 SAD 一致）。

**四、本轮遗留（交编排层）**

| # | 事项 | 性质 |
|---|------|------|
| 1 | 部署侧**可选**强制设 `PUBLIC_BASE_URL`（使 feed 键恒为 5 条 path）——登记为运营缓解，**非契约** | 运营（备注） |
| 2 | `_PROGRESS.md` / 变更日志登记 F8 | 契约同步（编排层） |

**本轮版本**：SAD v1.9 → **v1.10**；`doc/arch/tech-stack.json` 同步 `version: 1.10`（**内容有实质变更**：`backend.cache` / `namingRules`）。**约束保持**：Python 3 标准库零依赖、`layerIsolation`、文件结构（扁平包 + 唯一新增 `metrics.py`）均未放宽。**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`。**

### 9.11 v1.11（AC 落号对齐 + `last_modified=None` 来源补全 · 无设计变更）

> **性质**：PRD 已升至 **v0.9（AC 总数 36：高效 10 / 准确 14 / 稳定 12）**，SAD 侧因并行产出出现 AC 落号漂移——§8 把 **F2** 挂 `AC-A13`、**F8** 挂 `AC-S9`，覆盖声明仍记"无新增 AC 编号（仍 34 条）"。本轮据 PRD v0.9 收口，范围严格限定 `doc/arch/SAD.md` + `doc/arch/tech-stack.json`；**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`**。**无设计变更**（仅 AC 编号/计数对齐 + 一处 `None` 来源补全）；**只增不删编号**。

**一、改动条目**

| # | 项 | 内容 | 落点 |
|---|----|------|------|
| 1 | **AC 落号对齐** | **F2 → AC-A14**（原挂 `AC-A13`；AC-A13 仍承接 F1/F3/F4/F5/F6 与条件请求总语义）；**F8 → AC-S12**（主承接；`AC-S9`「24h 资源总账」保留为并列）；新增 **AC-A14 / AC-S12** 独立追溯行 | §8 追溯矩阵 / §8 AC 覆盖核对表 |
| 2 | **覆盖声明对齐** | AC 总数 **34 → 36**：**v1.9 承接 AC-A14（→ 35）**、**v1.10 承接 AC-S12（→ 36）**；⚠️ 仍 5 条（E4/A3/A8/S3/S4）；全文**现行**计数表述统一为 **36**（历史 §9.x 记录块保留原文，其"当时 34 条"为历史快照） | §8 覆盖声明；头部「上游输入」（PRD v0.9 / 36 AC） |
| 3 | **`last_modified=None` 来源补全** | 除"降级/回源失败"外，补 **"legacy / 非六字段缓存条目缺 `last_modified` 字段"**（P2-1 fail-safe：`entry.get('last_modified')` ⇒ `None` ⇒ **不发 `Last-Modified`、禁用 IMS**；`If-None-Match` 仍按 ETag 评估；**绝不抛 `KeyError` 经 `_guard` 变降级体**） | §2.7 / §7.1 N4 |

**二、文件动作**

| # | 文件 | 动作 |
|---|------|------|
| 1 | `doc/arch/SAD.md` | **v1.10 → v1.11**：头部版本 + v1.11 修订依据；§2.2 R-7 补 AC-S12 承接；§2.7 补 `None` 第二来源；§4.3 锁表行 AC 归属；§5 ADR-023 AC 归属注；§7.1 N4 补记；§8 追溯矩阵（F2/F8 行 + AC-A14/AC-S12 独立行 + 覆盖声明 34 → 36）；§9 头部 + 本节。**无设计变更、只增不删。** |
| 2 | `doc/arch/tech-stack.json` | **`version: 1.10 → 1.11`**：`backend.cache` 补 P2-1 fail-safe（legacy / 非六字段条目缺 `last_modified` ⇒ `entry.get`/`prev.get` 读 `None`、不发 `Last-Modified`/禁用 IMS，不抛 `KeyError`）。`allowlist`/`layerIsolation`/`fileStructure`/`importRestrictions` 不变。 |

**三、与 PRD / 详设的一致性核对（无冲突）**

1. PRD v0.9 §9「AC 总数 36（高效 10 / 准确 14 / 稳定 12）」、§3.2 **AC-A14**（feed 缓存键覆盖表示的全部变化维度）、§3.3 **AC-S12**（feed 取数锁表随并发收敛）与 SAD 本版逐条一致。
2. `server.md` **v1.11**（BR-SRV-38 措辞收口：`last_modified=None` 第二来源）/ `cache.md` **v1.10**（BR-CACHE-32/33 `.get` fail-safe）/ `server.md` **v1.10**（BR-SRV-50）/ `cache.md` **v1.9**（BR-CACHE-35）与 SAD 本版一致；测试 `SRV-T65`（F2）/ `SRV-T71`·`T72`（F8）/ `T-CACHE-35`（F8）。
3. **无设计变更**：§2.2 **R-7**、§2.7 条件请求语义、`BR-SRV-45` 放大论证边界（F2 引入的本地资源界由 AC-S12 提供）均未改动。

**四、本轮遗留（交编排层）**

| # | 事项 | 性质 |
|---|------|------|
| 1 | 变更日志登记「AC 落号对齐（F2→A14 / F8→S12，总数 34→36）+ §2.7 `None` 第二来源」 | 契约同步（编排层） |

**本轮版本**：SAD v1.10 → **v1.11**；`doc/arch/tech-stack.json` 同步 `version: 1.11`。**约束保持**：Python 3 标准库零依赖、`layerIsolation`、文件结构（扁平包 + 唯一新增 `metrics.py`）均未放宽。**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`。**

---

### 9.12 v1.12（台账收口 + 措辞更正 · 无设计变更）

> **性质**：编排层本轮取证后，把 SAD 中**已闭环却仍标"待编排层/待同步/待裁决"**的登记按证据收口，并订正一处失效数值口径。范围严格限定 `doc/arch/SAD.md` + `doc/arch/tech-stack.json`；**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`**（`API.md` 仅作**引用证据**）。**无设计变更、只增不删编号**；历史 §9.x 记录块原文保留（其"待同步/待裁决"为**当时快照**）。

**一、改动条目（逐项：证据 → SAD 落点）**

| # | 项 | 证据（本轮核实） | 旧 → 新 | SAD 落点 |
|---|----|-----------------|---------|---------|
| 1 | **Q2 `feeds[].status` 契约漂移** | `API.md:499-511` 明文列取值 `configured`/`ok`/`error`/`requires_chrome_cdp`，并写明 `/stock/data`·`/stock/basic_info` = `configured`（纯 REST）、`/stock/f10` = `requires_chrome_cdp`（即详设 `server.md` §2.9.1 三处目标值）；`API.md:226` `/stock/f10` 标 ⚡、`:244` `/stock/basic_info` 已无 ⚡；旁证：`doc/detailed/server.md` **v1.15** 已同轮把「待编排层批准/同步」改为「✅ 已批准并同步（`API.md:499-511`）」（仅引用，未修改该文件） | 状态 **"待同步" → "✅ 已同步（`API.md:499-511`）"**；**"同步权归编排层" → "已由编排层行使"** | §7.1 Q2 行 / "CDP 标注契约同步项"表 + 结论 / §2.6 契约口径 / §3.1 既有字段行 / 端锁定 |
| 2 | **§8 S4 状态** | Q2 唯一残留项（`API.md` `/stock/basic_info` 标注）已闭环（同上）；D-4 三分与 D-4 补充 2 均已在 v1.4 落地 | **S4：⚠️ → ✅**；§8 覆盖声明 **⚠️ 数 5 → 4**（余 E4/A3/A8/S3，均为待 P6c 实测/校准项） | §8 S4 行 / §8 覆盖声明 |
| 3 | **D-6 / S6 / ADR-014 残留措辞** | `ADR-014` 与 §2.3 D-6 已于 v1.5 写"正式反转确认"；§8 S6 仍只写"v1.4 反转" | **"（ADR-014 v1.4 反转）" → "（ADR-014 v1.4 反转 / v1.5 编排层正式确认）"** | §8 S6 行（历史 §9.4/§9.5 记录块保留原文） |
| 4 | **面板 `max-age` 域** | `API.md` §6.1「响应头口径」表（`API.md:619-631`）明示 4 面板跟随 `quote` 域、**盘中 4s / 非盘 120s**，并附实测 `/finance/market` → `max-age=120`（非盘）；`quote` 于 v1.4 升 L0 | SAD §3 server 行原仅写"`_cache_age()` 改读 policy + 未登记路径回落 `f10`"，**未写面板域**；本轮**补写**"4 面板注册 `quote` 域 ⇒ 盘中 4s / 非盘 120s"（订正历史"8/120"旧值），并指向 `API.md` §6.1 为对外权威 | §3 server.py 行 |
| 5 | **`stock_api → cdp_engine` 同层边** | `tech-stack.json` `layerIsolation` **已含**该边：`stock_api.py` 条目 `forbiddenImports` 仅 `server`/`stream` 且 `reason` 登记"允许同层单向引用 `cdp_engine.page_data`"；`cdp_engine.py` 条目反向 `forbiddenImports` 含 `stock_api` | **核实结论：已含，无需补边**；在 §3 留痕确认（无规则内容变更） | §3 同层依赖登记段 |
| 6 | **`tech-stack.json` 版本** | — | `version: 1.11 → 1.12`（**无 `allowlist`/`layerIsolation`/`namingRules`/`fileStructure` 内容变更**） | `doc/arch/tech-stack.json` |

**二、本轮确实未闭环（如实保留，未按"已完成"改写）**

| # | 事项 | 性质 |
|---|------|------|
| 1 | **§7.1 N4**：`API.md` 对 RSS 条件请求的**其余同步项**（`http_304_total` 观测名 / feed 条目四→六字段**形态变更** / HTTP/1.0 304 EOF 说明）在 `API.md` 无显式落点。**已同步部分**：`Last-Modified` = 表示最后一次变更时刻、跨 TTL 指纹未变仍 304、两条条件通道对等（`API.md:30`），且旧句"仅带 IMS 跨 TTL 完整重取"已不存在 | 契约同步（编排层，**部分未闭环**） |
| 2 | **AR-10 / P1-7**：PRD §9 的"51 用例全绿"表述待编排层同步 | 契约同步 |
| 3 | **AR-9 / AR-13 / Q6**：AC-S3 模式 B 分档 P95（高密度 ≤5s / 低密度 ≤10s）与"慢而未死"上游恢复延迟 ≤600s 需 P6c 实测校准（§8 S3 ⚠️ 未变） | P6c 校准 |
| 4 | **§8 E4 / A3 / A8**：命中率观测项（Q1）、条件 C1 实测（P1-1）、TTL 口径测试同步（Q3）仍待落地 | 测试落地 |
| 5 | **§9.10 遗留**：部署侧强制设 `PUBLIC_BASE_URL`（可选运营缓解，非契约）、`_PROGRESS.md`/变更日志登记 F8 | 运营/登记 |
| 6 | **§9.11 遗留 / 本轮新增**：变更日志登记（Q2 契约同步、S4 状态、AC 落号对齐等） | 契约同步（编排层） |

**三、tech-stack.json 动作**：`version: 1.11 → 1.12`（**仅版本号；内容零变更**——§3 提出的 `stock_api → cdp_engine` 同层边经核实**已存在**，无需补登）。**约束保持**：Python 3 标准库零依赖、`layerIsolation`、文件结构（扁平包 + 唯一新增 `metrics.py`）均未放宽。**未改代码、未改详设、未改 PRD。**

---

### 9.13 v1.13（溯源收口 + 引用时点约定 · 无设计变更）

> **性质**：本轮为**跨文档版本引用收口**，使 `doc/arch/` 与详设侧刚确立的「引用时点」口径一致。范围严格限定 `doc/arch/SAD.md` + `doc/arch/tech-stack.json`；**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`**。**无设计变更**（仅版本引用与约定口径）；历次修订依据块与 §9.1–§9.12 记录块中的版本引用为**历史快照，原文保留**。

**一、改动条目**

| # | 项 | 旧 → 新 | 落点 |
|---|----|---------|------|
| 1 | **上游 PRD 版本** | `v0.9 → **v0.10**`（PRD v0.10 仅同步用例 / 断言计数口径：现行基线 **514 单元用例全绿**，并显式区分「详设设计编号 75 条 ↔ 实测 514 方法」；**无对外字段 / 头项 / 状态码变更** ⇒ SAD 无设计变更） | 头部「上游输入」 |
| 2 | **新增「版本引用时点约定」** | 无 → 「**版本栏 = 最后一次同步时点的快照；内容以被引文档头部/正文为准、此栏仅用于定位；落后一版不属漂移**」 | 头部「上游输入」下方 |
| 3 | **文末承接清单（详设版本）** | `server.md` v1.11 / `cache.md` v1.10 / `metrics.md` v1.5 → **现行 `server.md` v1.17 / `cache.md` v1.15 / `metrics.md` v1.9 / `config.md` v1.7 / `stock_api.md` v1.4 / `market_api.md` v1.7 / `cdp_engine.md` v1.4 / `stream.md` v1.6**（一律以详设当前头部为准） | 文末承接清单 |
| 4 | **§6 tech-stack.json 版本引用** | `（v1.8）→（**v1.13**；v1.9–v1.11 内容变更见 §9.9–§9.11）` | §6 |
| 5 | **`tech-stack.json` 版本** | `version: 1.12 → **1.13**`（**仅版本号；`architectureRules`/`allowlist`/`layerIsolation`/`namingRules`/`fileStructure` 零变更**） | `doc/arch/tech-stack.json` |

**二、核对结论（无现行错误）**

1. **AC 计数**：现行 **36 条**（高效 10 / 准确 14 / 稳定 12；v0.6 增 E10/A11/A12、v0.7 增 A13、v0.8 增 A14、v0.9 增 S12）——§8 覆盖声明与头部「上游输入」均为 **36**；未发现把 30 / 34 / 35 当作**现行**计数的表述（相关数字仅出现在 §8/§9 的历史演进叙述与「30/30 一刀切」的历史更正注中）。
2. **指标注册表**：现行 **20 名**（v1.9 起新增 `http_304_total`）——§2.6 计分板 / §3 metrics 行 / §8 F3 行均为「19 → 20」，未发现把**现行**名数写成 19 的表述。
3. **详设头部真值（本轮核实，以文件头部为准）**：`server.md` v1.17 · `cache.md` v1.15 · `metrics.md` v1.9 · `config.md` v1.7 · `stock_api.md` v1.4 · `market_api.md` v1.7 · `cdp_engine.md` v1.4 · `stream.md` v1.6；上游 PRD **v0.10**。**任务书给定的对照表为较早快照**（`server.md` v1.16 / `cache.md` v1.14 / `metrics.md` v1.8 / `config.md` v1.6 / `stock_api.md` v1.3 / `market_api.md` v1.6 / `cdp_engine.md` v1.3 / `stream.md` v1.5）——本轮以**文件头部真值**为准收口（详设侧同轮对头部各自再进一版：`config.md` v1.6→v1.7 / `cache.md` v1.14→v1.15 / `metrics.md` v1.8→v1.9 / `server.md` v1.16→v1.17 / `stream.md` v1.5→v1.6 / `stock_api.md` v1.3→v1.4 / `market_api.md` v1.6→v1.7 / `cdp_engine.md` v1.3→v1.4）。
4. **约定一致性**：各详设（`server`/`cache`/`metrics`/`config`/`stock_api`/`market_api`/`cdp_engine`/`stream`）头部「上游 SAD / PRD」栏均已落 **v1.12 / v0.10**，并各自附「引用时点约定」注（口径与本档一致）；本档升 v1.13 后其上列栏将**落后一版**——按约定**不属漂移、无需每次追平**，内容以各详设头部为准。

**三、本轮遗留（交编排层）**

| # | 事项 | 性质 |
|---|------|------|
| 1 | 变更日志登记「SAD 溯源收口（PRD v0.10 / 文末详设清单）+ 引用时点约定」 | 契约同步（编排层） |

**本轮版本**：SAD v1.12 → **v1.13**；`doc/arch/tech-stack.json` 同步 `version: 1.13`。**约束保持**：Python 3 标准库零依赖、`layerIsolation`、文件结构（扁平包 + 唯一新增 `metrics.py`）均未放宽。**未改代码、未改详设、未改 PRD、未改 `API.md`/`README.md`/`.opencode/**`/`opencode.json`。**
...
| ② | **共享 URL 缓存 = 真正大头** | `cache.MAX_CACHE_SIZE=2000`（`cache.py:35`）；实测 `cache_entries.url=2000` **顶满**；条目为**解析后的 Python 对象**，实际占用远超 §4.3 按原始 ~10KB/条的"URL 20MB"报价 |
| ③ | 线程池 **glibc arena 碎片** | 20 批 worker 高频分配/释放；缺口第二候选 |

**三、新增架构事实：「缓存上界 ≠ 进程 RSS」（CR-01 预警被实测证实）**

§4.3 推导的**已知缓存项**上界 ≈**278MB** 与实测 python RSS **`1.012GiB`（≈1036MiB）** 相差 **≈750MB**。**含义**：该账目**只覆盖已知缓存项**（URL/feed/端点/队列/`_last_known`），**未覆盖分配器（glibc arena）/CDP/运行时开销**。**由此确立**：① **「调小 `cache_max` ⇒ 进程内存线性下降」的推断不成立**；② §4.3 一律标注为「**已知缓存项上界**」，**禁止**当作进程总内存上界（§2.2 R-4 / §4.3 口径声明 / §8 S9 同步）。**登记为待解架构项 AR-18**（§7.2；后续方向：**共享 URL 缓存调优**）。
